...
$ResultDir    = Join-Path $RepoRoot "tools\unity-check-results"
$ResultValidator = Join-Path $RepoRoot "tools\validate_unity_test_results.py"
...
$PythonPrefixArgs = @()
if ($Full) {
  if (-not (Test-Path $ResultValidator)) { throw "Missing: $ResultValidator" }
  ...
  if (Test-Path $venvPython) {
    ...
    Write-Error "-Full requires Python 3 to validate Unity test result XML."
    exit 2
  }
}
...
if ($Docker) {
  ...
  if (-not $env:UNITY_LICENSE) {
    ...
    Write-Host "       docker run --rm -v `"`${PWD}:/work`" unityci/editor:ubuntu-6000.0.75f1-base-3 ``"
    ...
    Write-Host ""
    Write-Host "(Keep UNITY_LICENSE local. This repository does not store Unity license material in GitHub Actions;"
    Write-Host "one local .ulf works across all matrix versions in practice.)"
    exit 2
  }
} else {
...
if ($Full) { Write-Host "  Results: $ResultDir" }
Write-Host ""

function Invoke-TestResultValidator([string]$ResultFile, [string]$LogFile) {
  $previousErrorActionPreference = $ErrorActionPreference
  try {
    # Windows PowerShell turns redirected native stderr into ErrorRecord objects.
    # Keep a validator failure non-terminating so its exit code can drive the
    # runner's normal per-version failure path.
    $ErrorActionPreference = "Continue"
    $validationOutput = & $PythonExe @PythonPrefixArgs $ResultValidator $ResultFile 2>&1
    $validationExitCode = $LASTEXITCODE
  } finally {
    $ErrorActionPreference = $previousErrorActionPreference
  }
  $validationOutput | Add-Content -LiteralPath $LogFile
...
function Invoke-LocalUnity([string]$Version, [string]$LogFile) {
  ...
  $resultFile = Join-Path $ResultDir "$Version.xml"
  if ($Full) {
    if (Test-Path -LiteralPath $resultFile) { Remove-Item -LiteralPath $resultFile }
    # The command-line test runner exits Unity after writing the result. Passing
    # -quit here can terminate the Editor before the test framework starts.
    $unityArgs = @("-batchmode", "-nographics", "-projectPath", $ProjectPath, "-runTests", "-testPlatform", "editmode", "-testResults", $resultFile, "-logFile", $LogFile)
  } else {
  ...
  if ($Full -and -not (Invoke-TestResultValidator $resultFile $LogFile)) { return 1 }
  return 0
}
...
function Invoke-DockerUnity([string]$Version, [string]$LogFile) {
  ...
  $resultFile = Join-Path $ResultDir "$Version.xml"
  if ($Full -and (Test-Path -LiteralPath $resultFile)) {
    Remove-Item -LiteralPath $resultFile
  }
  $unityExtra = if ($Full) {
    "-runTests -testPlatform editmode -testResults /results/$Version.xml"
  } else {
    "-quit"
  }
