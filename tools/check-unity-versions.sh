...
RESULT_DIR="${REPO_ROOT}/tools/unity-check-results"
RESULT_VALIDATOR="${REPO_ROOT}/tools/validate_unity_test_results.py"
...
if [[ $FULL -eq 1 ]]; then
  if ! command -v python3 >/dev/null 2>&1; then
    echo "error: --full requires Python 3 to validate Unity test result XML" >&2
    exit 2
  fi
  if [[ ! -f "$RESULT_VALIDATOR" ]]; then
    echo "error: result validator missing: $RESULT_VALIDATOR" >&2
    exit 2
  fi
fi

mkdir -p "$LOG_DIR"
[[ $FULL -eq 1 ]] && mkdir -p "$RESULT_DIR"
...
(Keep UNITY_LICENSE local. This repository does not store Unity license material in GitHub Actions.
One local .ulf works across all matrix versions in practice.)
EOF
    exit 2
  fi
...
echo "  Logs:    $LOG_DIR"
[[ $FULL -eq 1 ]] && echo "  Results: $RESULT_DIR"
echo
...
validate_results() {
  ...
  python3 "$RESULT_VALIDATOR" "$result_file" >>"$log_file" 2>&1
}

run_local() {
  local version="$1" log_file="$2"
  local unity_bin="${HUB_ROOT}/${version}/${UNITY_RELPATH}"
  local result_file="${RESULT_DIR}/${version}.xml"
  ...
  if [[ $FULL -eq 1 ]]; then
    ...
    args=(-batchmode -nographics -projectPath "$PROJECT_PATH" -runTests -testPlatform editmode -testResults "$result_file" -logFile "$log_file")
  else
    args=(-batchmode -quit -nographics -projectPath "$PROJECT_PATH" -logFile "$log_file")
  fi

  if "$unity_bin" "${args[@]}" >/dev/null 2>&1; then
    if [[ $FULL -eq 1 ]] && ! validate_results "$result_file" "$log_file"; then
      return 1
    fi
    return 0
  else
...
run_docker() {
  ...
  if [[ $FULL -eq 1 ]]; then
    rm -f -- "$result_file"
    unity_extra="-runTests -testPlatform editmode -testResults /results/${version}.xml"
  else
    unity_extra="-quit"
  fi
