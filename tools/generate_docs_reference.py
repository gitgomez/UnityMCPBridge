#!/usr/bin/env python3
"""
Generate Markdown reference pages for MCP for Unity tools and resources.
...
Outputs:
  docs/reference/tools/<group>/<tool-name>.md   — one per tool
  docs/reference/tools/<group>/index.md         — group landing
  docs/reference/tools/index.md                 — catalog landing
  docs/reference/resources/index.md             — resources catalog
...
TOOLS_OUT = DOCS_ROOT / "reference" / "tools"
RESOURCES_OUT = DOCS_ROOT / "reference" / "resources"
...
def _write(path: Path, content: str) -> bool:
    ...
    if path.exists() and path.read_text(encoding="utf-8") == content:
        return False
    # Keep generated references byte-stable across Windows and Unix. Without an
    # explicit newline policy, --check reports every page as stale on Windows.
...
def generate(
    ...
    # Per-tool pages + per-group landing pages.
