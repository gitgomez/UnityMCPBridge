...
# boot (fatal_after = boot_start + 120 s).
LICENSE_GRACE_S = 120
...
def _target_path(platform: str, *parts: str) -> str:
    """Join a path with the syntax of the platform being resolved."""
...
def hub_roots(platform: str | None = None, environ: dict[str, str] | None = None) -> list[str]:
    ...
    if plat.startswith("win"):
        ...
        for var in ("ProgramFiles", "ProgramFiles(x86)"):
            base = env.get(var)
            if base:
                roots.append(_target_path(plat, base, "Unity", "Hub", "Editor"))
        if not roots:
            roots.append(r"C:\Program Files\Unity\Hub\Editor")
        return roots
    # linux + everything else
    return [_target_path(plat, home, "Unity", "Hub", "Editor")]
...
def read_secondary_install_path(platform: str | None = None,
    ...
    home = env.get("HOME") or env.get("USERPROFILE") or str(Path.home())
    if plat == "darwin":
        cfg = _target_path(plat, home, "Library", "Application Support", "UnityHub", "secondaryInstallPath.json")
    elif plat.startswith("win"):
        appdata = env.get("APPDATA") or _target_path(plat, home, "AppData", "Roaming")
        cfg = _target_path(plat, appdata, "UnityHub", "secondaryInstallPath.json")
    else:
        xdg = env.get("XDG_CONFIG_HOME") or _target_path(plat, home, ".config")
        cfg = _target_path(plat, xdg, "UnityHub", "secondaryInstallPath.json")
...
def candidate_editor_paths(version: str,
    ...
    for root in hub_roots(plat, env):
        out.append(_target_path(plat, root, version, relpath))

    sec = read_secondary_install_path(plat, env, read_text)
    if sec:
        out.append(_target_path(plat, sec, version, relpath))
    ...
    for root in roots:
        ...
        for name in entries:
            ...
            if (pv[0], pv[1]) != (target[0], target[1]):
                continue
            binary = _target_path(plat, root, name, relpath)
