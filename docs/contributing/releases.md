...
3. Synchronize every release-owned version and install URL:
   ...
   ```bash
   python tools/update_versions.py --version 10.2.4
   python tools/update_versions.py --check --version 10.2.4
   ```
...
```bash
git tag -a v10.2.4 <release-commit> -m "UnityMCPBridge v10.2.4"
git show --no-patch --decorate v10.2.4
...
```text
https://github.com/gitgomez/UnityMCPBridge.git?path=/MCPForUnity#v10.2.4
