...
A tool's `group` parameter controls per-session visibility — see [Tool Groups](../guides/tool-groups.md). `group=None` means the tool is always visible (server meta-tools like `set_active_instance` and `manage_tools`).
