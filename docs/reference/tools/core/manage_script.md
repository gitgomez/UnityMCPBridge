...
<!-- examples:start -->
> [!NOTE]
> **Use the right tool for the job.** `manage_script` only handles whole-file
> lifecycle: **create**, **read**, and **delete**. For editing existing scripts,
> use [`script_apply_edits`](./script_apply_edits.md) for structured edits,
> [`apply_text_edits`](./apply_text_edits.md) for surgical text patches,
> [`validate_script`](./validate_script.md) for Roslyn validation, and
> [`read_console`](./read_console.md) for compile diagnostics.
...
Unity needs a domain reload to compile the new file (or notice the old one is gone). Poll the `editor_state` resource's `isCompiling` field until it flips back to `false`, then run [`read_console`](./read_console.md) to catch any compile errors before relying on the new types.
<!-- examples:end -->
