...
| `search_pattern` | `str \| None` | — | One Unity AssetDatabase query. Whitespace-separated terms are combined by Unity as one query (typically AND), e.g. 'Player t:Prefab'. Globs such as '*.prefab' are not supported. |
| `search_patterns` | `list[str] \| str \| None` | — | Alternative Unity AssetDatabase queries combined with OR. Pass a JSON array/list such as ['Player', 'Enemy']; cannot be combined with search_pattern. |
