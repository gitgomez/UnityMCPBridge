...
| `target` | `str \| dict[str, Any] \| None` | — | GameObject identifier by name, path, instance ID, or Runtime v1 scene-object handle for modify/delete/duplicate actions |
...
| `parent` | `str \| dict[str, Any] \| None` | — | Parent GameObject reference or Runtime v1 scene-object handle - used for create/modify |
...
| `reference_object` | `str \| dict[str, Any] \| None` | — | Reference object for relative movement (required for move_relative) |
...
| `look_at_target` | `list[float] \| str \| dict[str, Any] \| None` | — | World position [x,y,z], GameObject name/path/ID, or Runtime v1 handle to look at. |
| `look_at_up` | `list[float] \| str \| None` | — | Optional up vector [x,y,z] for look_at. Defaults to [0,1,0]. |
| `if_match` | `dict[str, Any] \| None` | — | Optional Runtime v1 state precondition from a prior response.state |
...
See [Multi-Instance Routing](../../../guides/multi-instance.md) for the full routing model.
<!-- examples:end -->
