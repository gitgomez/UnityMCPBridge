...
| `dry_run` | `bool \| None` | — | Validate the full batch without executing child commands |
| `atomicity` | `Literal['none', 'undo_group'] \| None` | — | Optional Unity Undo grouping for eligible scene commands |
| `rollback_on_failure` | `bool \| None` | — | Revert completed Undo-compatible commands after a child failure; requires atomicity='undo_group' |
