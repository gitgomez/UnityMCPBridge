...
The CLI preserves the active terminal encoding. On legacy Windows encodings,
status icons automatically use ASCII markers such as `[OK]`, while other
unsupported response characters are emitted as backslash escapes. Commands do
not require a separate `PYTHONUTF8` override.
