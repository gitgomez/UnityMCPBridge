...
> [!CAUTION]
> **Switching transport requires a restart.** If you change from `http` to
> `stdio` (or vice versa) in the MCP for Unity window, **restart Claude Code**
...
- macOS dyld ICU library errors: see [Common Setup Problems → macOS Claude CLI dyld error](./troubleshooting.md#macos-claude-cli-fails-to-start-dyld-icu-library-not-loaded)
- "Claude Not Found" in the Register button: see the FAQ in [Common Setup Problems](./troubleshooting.md#faq--claude-code)
