...
## Fork package is installed but fork-only commands are missing

The UnityMCPBridge fork changes both the Unity package and the Python server. Stable fork packages derive the matching tagged GitHub server automatically; older installs or a saved override can still point elsewhere.

Open **Window → MCP for Unity → Advanced Settings**. Clear an unintended override to restore the matching tagged default, or set it deliberately to a matching fork `Server` checkout or Git source. Reconfigure/restart the affected MCP client. See [UnityMCPBridge Fork](../getting-started/bridge-fork.md) for exact sources and verification.

---

## Receipt ledger is full or normal commands are rejected

Use the administrative CLI path, which does not depend on normal receipt admission:
...
The default cleanup is conservative: it deletes only expired terminal receipts and never deletes `accepted`, `queued`, or `executing`. Deleting all terminal receipts requires `--all-terminal`; deleting `outcome_unknown` additionally requires `--confirm-outcome-unknown`. If persistence fails, no in-memory deletion is applied. Follow the [safe recovery procedure](../getting-started/bridge-fork.md#safe-receipt-ledger-recovery) before retrying.
...
> [!TIP]
> **When in doubt, restart your client.** Clients like Claude Code or JetBrains
> Rider can get confused if you switch transport modes mid-session. Restart the
> client so it picks up the new configuration.
...
A: Install the CLI or set the path. Click the orange **[HELP]** link in the MCP for Unity window for step-by-step install instructions, then choose the binary location. See also: [Install or Repair Claude Code CLI](./claude-code-cli.md).
