...
8. [Administrative Receipt Recovery](#administrative-receipt-recovery)
9. [Runtime UI Interaction](#runtime-ui-interaction)
10. [Command Reference by Category](#command-reference-by-category)
...
Receipt diagnosis and cleanup use a separate administrative path so they remain available when normal command admission is blocked.

```bash
# Read-only counts by lifecycle state
unity-mcp receipts status

# Default: delete only expired terminal receipts
unity-mcp receipts cleanup

# Explicitly delete all known terminal receipts regardless of age
unity-mcp receipts cleanup --all-terminal

# Separately acknowledge uncertain side effects before deleting outcome_unknown
unity-mcp receipts cleanup --confirm-outcome-unknown
unity-mcp receipts cleanup --all-terminal --confirm-outcome-unknown
```

`accepted`, `queued`, and `executing` are never deleted. `outcome_unknown` is always reported separately and retained unless explicitly confirmed. Cleanup persists the retained snapshot before replacing in-memory state. If persistence fails with `RECEIPT_CLEANUP_PERSIST_FAILED`, memory remains unchanged; fix persistence, run `receipts status`, and retry.

For implementation invariants, see `docs/development/COMMAND_RUNTIME_V1.md`.
...
```bash
unity-mcp editor play-ui-status
...
unity-mcp editor set-ui-toggle --help
unity-mcp editor hover-ui --help
unity-mcp editor key-ui --help
```
