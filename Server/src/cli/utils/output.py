...
def configure_console_output() -> None:
    """Keep the active console encoding while making writes lossless and safe.

    Windows Python can expose a legacy encoding such as cp1252 even when the
    terminal itself uses UTF-8. Unsupported response text is escaped instead of
    terminating the CLI with UnicodeEncodeError.
    """
    for stream in (sys.stdout, sys.stderr):
        ...
        try:
            ...
            # Some embedded or redirected streams cannot be reconfigured. The
            # status helpers below still use encoding-safe ASCII markers.
            continue
...
def _select_marker(
    ...
    *,
    ...
    """Select a readable marker supported by the active output stream."""
...
def format_as_text(data: Any, indent: int = 0) -> str:
    ...
    if isinstance(data, dict):
        ...
        if "success" in data and not data.get("success"):
            ...
            marker = _select_marker("❌ Error:", "[ERROR]")
            return f"{prefix}{marker} {error}"
...
def print_success(message: str) -> None:
    ...
    marker = _select_marker("✅", "[OK]")
    click.echo(f"{marker} {message}")


def print_error(message: str) -> None:
    """Print an error message to stderr."""
    marker = _select_marker("❌", "[ERROR]", err=True)
    click.echo(f"{marker} {message}", err=True)


def print_warning(message: str, *, err: bool = False) -> None:
    """Print a warning message."""
    marker = _select_marker("⚠️", "[WARNING]", err=err)
    click.echo(f"{marker} {message}", err=err)


def print_info(message: str) -> None:
    """Print an info message."""
    marker = _select_marker("ℹ️", "[INFO]")
    click.echo(f"{marker} {message}")
