...
def warn_if_remote_host(config: CLIConfig) -> None:
    ...
    """
    from cli.utils.output import print_warning

    local_hosts = ("127.0.0.1", "localhost", "::1", "0.0.0.0")
    if config.host.lower() not in local_hosts:
        print_warning(
            "Security Warning: Connecting to non-localhost server.\n"
...
async def send_receipt_ledger_admin(
    *,
    action: str,
    ...
    """Use the local receipt-ledger control plane, outside command admission."""
    cfg = config or get_config()
    url = f"http://{cfg.host}:{cfg.port}/api/receipt-ledger"
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(url, json=payload, timeout=cfg.timeout)
            response.raise_for_status()
            return response.json()
    except httpx.ConnectError as e:
        raise UnityConnectionError(
        ...
        raise UnityConnectionError(
            f"Receipt ledger control timed out after {cfg.timeout}s."
        )
    except httpx.HTTPStatusError as e:
        raise UnityConnectionError(
            f"HTTP error from server: {e.response.status_code} - {e.response.text}"
...
def run_receipt_ledger_admin(
    *,
    ...
    """Synchronous wrapper for the receipt-ledger control plane."""
    return asyncio.run(
        send_receipt_ledger_admin(
            action=action,
            scope=scope,
            confirm_outcome_unknown=confirm_outcome_unknown,
            config=config,
