...
class UnityInstanceMiddleware(Middleware):
    ...
    async def _resolve_instance_value(self, value: str, ctx) -> str:
        ...
        if "@" in value:
            if value in ids:
                return value
            reloading = await self._resolve_reloading_instance(value, ctx)
            if reloading:
                return reloading
    ...
    async def _resolve_reloading_instance(self, value: str, ctx) -> str | None:
        """Preserve explicit HTTP routing while a negotiated reload is in flight."""
        ...
        reloading_ids = await PluginHub.get_reloading_instances(user_id=user_id)

        if "@" in value:
            return value if value in reloading_ids else None

        lookup = value.lower()
        matches = [
            instance_id
            for instance_id in reloading_ids
            if instance_id.rpartition("@")[2].lower().startswith(lookup)
        ]
        if len(matches) == 1:
            return matches[0]
        if len(matches) > 1:
            raise ValueError(
                f"Hash prefix '{value}' is ambiguous ({', '.join(matches)}). "
                "Provide the full Name@hash from mcpforunity://instances."
            )
        return None
