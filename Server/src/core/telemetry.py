...
class TelemetryConfig:
    ...
    def __init__(self):
        ...
        # Fork policy: no network telemetry without explicit opt-in and a valid
        # endpoint. An environment endpoint is itself an explicit opt-in;
        # application configuration must set both enabled and endpoint.
        cfg_enabled = False if server_config is None else bool(
            getattr(server_config, "telemetry_enabled", False))
        cfg_endpoint = None if server_config is None else getattr(
            server_config, "telemetry_endpoint", None)
        env_endpoint = os.environ.get("UNITY_MCP_TELEMETRY_ENDPOINT", "").strip()
        configured_endpoint = env_endpoint or cfg_endpoint
        self.endpoint = self._validated_endpoint(configured_endpoint)
        self.enabled = bool(self.endpoint) and (
            cfg_enabled or bool(env_endpoint)
        ) and not self._is_disabled()
        try:
            logger.info(
                f"Telemetry configured: enabled={self.enabled}, endpoint={self.endpoint or '<unset>'}, timeout_env={os.environ.get('UNITY_MCP_TELEMETRY_TIMEOUT') or '<unset>'}")
        except Exception:
            pass
    ...
    def _validated_endpoint(self, candidate: str | None) -> str | None:
        """Return a valid explicit telemetry endpoint, otherwise ``None``."""
        if not candidate:
            return None
        try:
            ...
            logger.debug(
                f"Invalid telemetry endpoint '{candidate}'; telemetry remains disabled. Error: {e}",
                exc_info=True,
            )
            return None
        ...
        try:
            ...
            if httpx:
                with httpx.Client(timeout=self.config.timeout) as client:
                    ...
                    endpoint = self.config._validated_endpoint(self.config.endpoint)
                    if endpoint is None:
                        return
...
def record_tool_usage(tool_name: str, success: bool, duration_ms: float, error: str | None = None, sub_action: str | None = None):
    ...
    Args:
        ...
        error: Accepted for API compatibility but intentionally not recorded.
...
def record_latency(operation: str, duration_ms: float, metadata: dict[str, Any] | None = None):
    """Record latency telemetry without caller-supplied free-form metadata."""
    data = {
        "operation": operation,
        "duration_ms": round(duration_ms, 2)
    }
...
def record_failure(component: str, error: str, metadata: dict[str, Any] | None = None):
    """Record a failure signal without error text or free-form metadata."""
    data = {
        "component": component
    }
