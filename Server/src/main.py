...
def create_mcp_server(project_scoped_tools: bool) -> FastMCP:
    ...
    if not config.http_remote_hosted:
        ...
        @mcp.custom_route("/api/receipt-ledger", methods=["POST"])
        async def cli_receipt_ledger_route(request: Request) -> JSONResponse:
            """Local administrative receipt-ledger control path."""
            try:
                body = await request.json()
                action = body.get("action", "diagnose")
                scope = body.get("scope", "expired_terminal")
                confirm_outcome_unknown = body.get(
                    "confirm_outcome_unknown", False
                )
                unity_instance = body.get("unity_instance")

                if action not in {"diagnose", "cleanup"}:
                    return JSONResponse(
                        {
                            "success": False,
                            "error": "Action must be 'diagnose' or 'cleanup'.",
                        },
                        status_code=400,
                    )
                if scope not in {"expired_terminal", "all_terminal"}:
                    return JSONResponse(
                        {
                            "success": False,
                            "error": (
                                "Scope must be 'expired_terminal' or "
                                "'all_terminal'."
                            ),
                        },
                        status_code=400,
                    )
                if not isinstance(confirm_outcome_unknown, bool):
                    return JSONResponse(
                        {
                            "success": False,
                            "error": "confirm_outcome_unknown must be a boolean.",
                        },
                        status_code=400,
                    )
                if unity_instance is not None and not isinstance(
                    unity_instance, str
                ):
                    return JSONResponse(
                        {
                            "success": False,
                            "error": "unity_instance must be a string.",
                        },
                        status_code=400,
                    )
                ...
                result = await PluginHub.manage_receipt_ledger(
                    session_id,
                    action=action,
                    scope=scope,
                ...
                logger.exception("CLI receipt ledger error: %s", exc)
                return JSONResponse(
                    {"success": False, "error": str(exc)},
                    status_code=500,
                )
