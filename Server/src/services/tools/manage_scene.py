...
async def manage_scene(
    ...
    # Closing a loaded scene is itself the recovery action recommended by the
    # external-YAML guard, so do not deadlock it behind a blocked refresh.
    refresh_if_dirty = (
        action not in read_only_actions and action != "close_scene"
    )
    if action == "validate":
        refresh_if_dirty = coerce_bool(auto_repair, default=False)
