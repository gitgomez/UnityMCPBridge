...
@mcp_for_unity_tool(
    ...
    description=(
        ...
        "Supports full preflight, dry_run, and truthful Unity Undo grouping for eligible scene-only batches. "
...
async def batch_execute(
    ...
    dry_run: Annotated[bool | None,
                       "Validate the full batch without executing child commands"] = None,
    atomicity: Annotated[Literal["none", "undo_group"] | None,
                         "Optional Unity Undo grouping for eligible scene commands"] = None,
    rollback_on_failure: Annotated[bool | None,
                                   "Revert completed Undo-compatible commands after a child failure; requires atomicity='undo_group'"] = None,
) -> dict[str, Any]:
    ...
    if atomicity is not None:
        payload["atomicity"] = atomicity
    if rollback_on_failure is not None:
        payload["rollbackOnFailure"] = bool(rollback_on_failure)
