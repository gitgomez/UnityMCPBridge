...
async def manage_prefabs(
    ...
    try:
        gate = await preflight(
            ...
            # Closing the stage is itself the recovery action recommended by the
            # external-YAML guard, so it must never be preceded by a blocked refresh.
            refresh_if_dirty=action not in {
                "get_info", "get_hierarchy", "close_prefab_stage"
            },
        )
        if gate is not None:
