    ...
    if refresh_if_dirty:
        ...
        if isinstance(assets, dict) and assets.get("external_changes_dirty") is True:
            try:
                ...
                compile_mode = (
                    "request"
                    if assets.get("external_script_changes_dirty") is True
                    else "none"
                )
                ...
                if (isinstance(refreshed_data, dict)
                        and refreshed_data.get("success") is False):
                    return MCPResponse(**refreshed_data)

                # Continue with a post-refresh snapshot instead of the stale state
                # captured before a possible compilation/domain reload.
                state_resp = await get_editor_state(ctx)
                state = (state_resp.model_dump()
                         if hasattr(state_resp, "model_dump") else state_resp)
                refreshed_state_data = (
                    state.get("data") if isinstance(state, dict) else None
                )
                if isinstance(refreshed_state_data, dict):
                    data = refreshed_state_data
            except Exception:
