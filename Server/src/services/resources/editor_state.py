...
class EditorStateExternalChangedPath(BaseModel):
    path: str
    kind: str
    source: str
    extension: str
    requires_script_compilation: bool
    is_unity_yaml: bool
    change_type: str | None = None


class EditorStateLastEditorSave(BaseModel):
    epoch: str
    sequence: int
    path: str
    mtime_unix_ns: int
    mtime_unix_100ns: int | None = None
...
class EditorStateAssets(BaseModel):
    ...
    external_script_changes_dirty: bool | None = None
    ...
    external_changed_paths: list[EditorStateExternalChangedPath] = Field(
        default_factory=list
    )
    external_changed_paths_truncated: bool = False
    last_editor_save: EditorStateLastEditorSave | None = None
    editor_save_receipts: list[EditorStateLastEditorSave] = Field(
        default_factory=list
    )
...
async def get_editor_state(ctx: Context) -> MCPResponse:
    ...
    try:
        ...
        if isinstance(instance_id, str) and instance_id.strip():
            ...
            receipts = assets.get("editor_save_receipts")
            if isinstance(receipts, list):
                external_changes_scanner.acknowledge_editor_saves(
                    instance_id,
                    [item for item in receipts if isinstance(item, dict)],
                )
            else:
                receipt = assets.get("last_editor_save")
                if isinstance(receipt, dict):
                    external_changes_scanner.acknowledge_editor_save(
                        instance_id, receipt
                    )
            ...
            assets["external_script_changes_dirty"] = bool(
                ext.get("external_script_changes_dirty", False))
            ...
            assets["external_changed_paths"] = ext.get(
                "external_changed_paths", [])
            assets["external_changed_paths_truncated"] = bool(
                ext.get("external_changed_paths_truncated", False)
            )
    except Exception:
        pass
