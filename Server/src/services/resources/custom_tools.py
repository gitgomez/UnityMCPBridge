...
async def get_custom_tools(ctx: Context) -> CustomToolsResourceResponse | MCPResponse:
    ...
    registered_tools = await service.list_registered_tools(
        project_id,
        user_id=user_id,
    )
    built_in_names = {tool["name"] for tool in get_registered_tools()}
    tools = [
        tool
        for tool in registered_tools
        if tool.is_built_in is not True and tool.name not in built_in_names
    ]
    ...
    return CustomToolsResourceResponse(
        success=True,
        message="Project-defined custom tools retrieved successfully.",
        data=data,
    )
