...
async def test_plugin_hub_waits_for_reconnection_during_reload():
    """PluginHub waits on the registry event for plugin reconnection."""
...
@pytest.mark.asyncio
async def test_send_command_for_instance_recovers_disconnect_with_same_receipt(monkeypatch):
    """A reload disconnect retries once with the same logical Runtime request."""
    from transport.plugin_hub import PluginHub

    resolve_mock = AsyncMock(side_effect=["session-before", "session-after"])
    ...
    result = await PluginHub.send_command_for_instance(
        unity_instance="Project@hash-reload",
        command_type="manage_asset",
        params={"action": "delete", "path": "Assets/Temp.asset"},
    ...
    retry_kwargs = send_mock.await_args_list[1].kwargs
    assert first_kwargs["attempt"] == 1
    assert retry_kwargs["attempt"] == 2
    assert first_kwargs["request_id"] == retry_kwargs["request_id"]


@pytest.mark.asyncio
async def test_fast_command_readiness_re_resolves_session_after_reload(monkeypatch):
    """Readiness probes must follow Unity's new session id after a reload."""
    from transport.plugin_hub import PluginHub

    resolve_mock = AsyncMock(side_effect=["session-before", "session-after"])
    ensure_mock = AsyncMock(side_effect=[True, False])
    send_mock = AsyncMock(side_effect=[
        {"runtime_version": 1, "status": "succeeded", "message": "pong"},
        {"success": True, "message": "State read after reload."},
    ])

    monkeypatch.setattr(PluginHub, "_resolve_session_id", resolve_mock)
    monkeypatch.setattr(PluginHub, "_ensure_live_connection", ensure_mock)
    monkeypatch.setattr(PluginHub, "send_command", send_mock)
    monkeypatch.setattr(PluginHub, "_FAST_FAIL_COMMANDS", {"get_editor_state"})
    monkeypatch.setenv("UNITY_MCP_SESSION_READY_WAIT_SECONDS", "1")
    ...
    assert result["success"] is True
    assert resolve_mock.await_count == 2
    assert resolve_mock.await_args_list[1].kwargs["retry_on_reload"] is False
    assert send_mock.await_args_list[0].args[0] == "session-after"
    assert send_mock.await_args_list[1].args[0] == "session-after"
