...
@pytest.mark.asyncio
async def test_get_gameobject_accepts_global_object_id(monkeypatch):
    """A stable ID copied from a handle is forwarded as a scene-object target."""
    captured = {}

    async def fake_send(cmd, params, **kwargs):
        captured["cmd"] = cmd
        captured["params"] = params
        return {"success": True, "data": {"instanceID": 24680, "name": "Player"}}
    ...
    global_object_id = (
        "GlobalObjectId_V1-2-0123456789abcdef0123456789abcdef-12345-0"
    )

    resp = await gameobject_res_mod.get_gameobject(
        ctx=DummyContext(),
        instance_id=global_object_id,
    )

    assert resp.success is True
    assert captured["cmd"] == "get_gameobject"
...
@pytest.mark.asyncio
async def test_get_gameobject_components_accepts_global_object_id(monkeypatch):
    captured = {}
    ...
    assert resp.success is True
    assert captured["params"]["target"]["global_object_id"] == global_object_id
    assert "instanceID" not in captured["params"]
...
@pytest.mark.asyncio
async def test_get_gameobject_rejects_invalid_object_reference():
    resp = await gameobject_res_mod.get_gameobject(
        ctx=DummyContext(),
        instance_id="PlayerByNameIsNotAResourceId",
    )

    assert resp.success is False
    assert "Invalid GameObject reference" in (resp.error or "")
