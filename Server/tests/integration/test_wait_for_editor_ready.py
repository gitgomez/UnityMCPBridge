...
@pytest.mark.asyncio
async def test_compile_wait_requires_a_new_compilation_cycle(monkeypatch):
    monkeypatch.delenv("PYTEST_CURRENT_TEST", raising=False)
    ...
    states = [
        ...
        {
            "sequence": 11,
            "advice": {"ready_for_tools": False, "blocking_reasons": ["compiling"]},
            "compilation": {"last_compile_started_unix_ms": 200},
        },
        {
            "sequence": 12,
            "advice": {"ready_for_tools": True, "blocking_reasons": []},
            "compilation": {
                ...
                "last_compile_finished_unix_ms": 300,
            },
        },
    ]
    call_count = 0
    ...
    baseline = {
        "sequence": 10,
        "compilation": {
            "last_compile_started_unix_ms": 100,
            "last_compile_finished_unix_ms": 150,
        },
    }
    ready, _, final_state = await mod._wait_for_editor_ready_state(
        DummyContext(),
        timeout_s=5.0,
        after_state=baseline,
        require_compilation_cycle=True,
    )
