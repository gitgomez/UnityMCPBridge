...
@pytest.mark.asyncio
async def test_manage_components_target_by_scene_object_handle(monkeypatch):
    """Stable handles pass through without lossy string coercion."""
    captured = {}
    ...
    assert resp.get("success") is True
    assert captured["params"]["target"] == handle
