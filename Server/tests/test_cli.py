...
class TestOutputFormatting:
    ...
    def test_status_helpers_use_ascii_markers_for_cp1252(self):
        stdout = MagicMock()
        stdout.encoding = "cp1252"
        stderr = MagicMock()
        stderr.encoding = "cp1252"

        with patch("cli.utils.output.sys.stdout", stdout), patch(
            "cli.utils.output.sys.stderr", stderr
        ), patch("cli.utils.output.click.echo") as echo:
            print_success("connected")
            print_error("failed")
            print_warning("careful")
            print_warning("remote", err=True)
            print_info("details")
            formatted_error = format_as_text(
                {"success": False, "error": "broken"}
            )

        assert echo.call_args_list == [
            (("[OK] connected",), {}),
            (("[ERROR] failed",), {"err": True}),
            (("[WARNING] careful",), {"err": False}),
            (("[WARNING] remote",), {"err": True}),
            (("[INFO] details",), {}),
        ]
        assert formatted_error == "[ERROR] broken"

    def test_configure_console_output_uses_backslash_replacement(self):
        stdout = MagicMock()
        stderr = MagicMock()
...
class TestCLICommands:
    ...
    def test_receipt_admin_cli_keeps_cleanup_scopes_and_confirmation_explicit(
        ...
        """Receipt cleanup defaults safe and forwards both destructive flags."""
        responses = [
            {"success": True, "ledger": {"total": 5}},
            {
                "success": True,
                "removed_total": 4,
                "preserved_outcome_unknown": 1,
            },
            {
                "success": True,
                "removed_total": 5,
                "preserved_outcome_unknown": 0,
            },
        ]
        with patch(
            "cli.commands.receipts.run_receipt_ledger_admin",
            side_effect=responses,
        ) as admin:
            status_result = runner.invoke(cli, ["receipts", "status"])
            safe_cleanup = runner.invoke(cli, ["receipts", "cleanup"])
            full_cleanup = runner.invoke(
                cli,
                [
                    "receipts",
                    "cleanup",
        ...
        assert safe_cleanup.exit_code == 0
        assert full_cleanup.exit_code == 0
        assert admin.call_args_list[0].kwargs["action"] == "diagnose"
        assert admin.call_args_list[1].kwargs["scope"] == "expired_terminal"
        assert admin.call_args_list[1].kwargs["confirm_outcome_unknown"] is False
        assert admin.call_args_list[2].kwargs["scope"] == "all_terminal"
        assert admin.call_args_list[2].kwargs["confirm_outcome_unknown"] is True

    @pytest.mark.parametrize(
        ("args", "unknown_command", "suggested_command"),
        [
            (["camra"], "camra", "camera"),
            (["camera", "relese"], "relese", "release"),
        ],
    )
    def test_unknown_command_reports_usage_with_suggestion(
        self,
        runner,
        args,
        unknown_command,
        ...
        """Unknown root and nested commands remain ordinary usage errors."""
        result = runner.invoke(cli, args)

        assert result.exit_code == 2
        assert f"No such command '{unknown_command}'." in result.output
        assert f"Did you mean: {suggested_command}" in result.output
...
class TestCameraCommands:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (["camera", "ping"], "ping"),
            (["camera", "list"], "list_cameras"),
            (["camera", "brain-status"], "get_brain_status"),
            (["camera", "create"], "create_camera"),
            (["camera", "ensure-brain"], "ensure_brain"),
            (["camera", "set-target", "Main Camera"], "set_target"),
            (["camera", "set-lens", "Main Camera"], "set_lens"),
            (
                ["camera", "set-priority", "Main Camera", "--priority", "10"],
                "set_priority",
            ),
            (["camera", "set-body", "Main Camera"], "set_body"),
            (["camera", "set-aim", "Main Camera"], "set_aim"),
            (["camera", "set-noise", "Main Camera"], "set_noise"),
            (
                [
                    "camera",
                    "add-extension",
            ...
            (["camera", "set-blend"], "set_blend"),
            (["camera", "force", "Main Camera"], "force_camera"),
            (["camera", "release"], "release_override"),
            (["camera", "screenshot"], "screenshot"),
            (["camera", "screenshot-multiview"], "screenshot_multiview"),
        ],
    )
    def test_camera_commands_use_standard_run_command_signature(
        self,
        runner,
        mock_unity_response,
        args,
        ...
        with patch(
            "cli.commands.camera.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
            result = runner.invoke(cli, args)

        assert result.exit_code == 0, result.output
        command_type, params, config = mock_run.call_args[0]
...
class TestEditorCommands:
    ...
    def test_editor_play_ui_status(self, runner, mock_unity_response):
        """Test runtime UI capability status command."""
        with patch(
            "cli.commands.editor.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
            result = runner.invoke(cli, ["editor", "play-ui-status"])
    ...
    def test_editor_inspect_ui_toolkit_element(self, runner, mock_unity_response):
        """Test bounded UI Toolkit element inspection."""
        with patch(
            "cli.commands.editor.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
        ...
        assert mock_run.call_args.args[:2] == (
            "interact_play_mode",
            {
                ...
                "document": "OperationsManagement",
                "element_name": "definition-scroll",
                "include_text": False,
            },
        )

    def test_editor_wait_ui_forwards_one_logical_command(self, runner):
        """Test CLI wait forwarding without per-poll transport commands."""
        wait_result = {
            "success": True,
            "data": {
                "condition": "visible",
                "attempts": 2,
                "state": {"exists": True, "visible": True},
            },
        }
        with patch(
            "cli.commands.editor.run_command",
    ...
    def test_editor_set_ui_text_toolkit(self, runner, mock_unity_response):
        """Test UI Toolkit text mutation forwarding."""
        with patch(
            "cli.commands.editor.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
    ...
    def test_editor_set_ui_toggle_ugui(self, runner, mock_unity_response):
        """Test uGUI toggle mutation forwarding."""
        with patch(
            "cli.commands.editor.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
        ...
        assert mock_run.call_args.args[:2] == (
            ...
            {
                "action": "set_toggle",
                "target": "Canvas/Settings/Telemetry",
                "value": False,
            },
        )

    def test_editor_click_ui_by_path(self, runner, mock_unity_response):
        """Test runtime UI target click command."""
        with patch(
            "cli.commands.editor.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
        ...
        assert mock_run.call_args.args[:2] == (
            ...
            {
                "action": "click_ui",
                "target": "Canvas/Menu/Start",
                "search_method": "by_path",
            },
        )

    def test_editor_click_ui_by_position(self, runner, mock_unity_response):
        """Test normalized runtime UI coordinate click command."""
        with patch(
            "cli.commands.editor.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
            result = runner.invoke(
                cli,
                ["editor", "click-ui", "--position", "0.5", "0.75"],
            )
    ...
    def test_editor_click_ui_requires_one_address(self, runner):
        ...
        with patch("cli.commands.editor.run_command") as mock_run:
            result = runner.invoke(cli, ["editor", "click-ui"])

        assert result.exit_code != 0
        assert "exactly one" in result.output
        mock_run.assert_not_called()

    def test_editor_click_ui_toolkit_by_element(self, runner, mock_unity_response):
        """Test UI Toolkit element click command."""
        with patch(
            "cli.commands.editor.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
    ...
    def test_editor_click_ui_toolkit_rejects_mixed_address(self, runner):
        """Reject a mixed uGUI/UI Toolkit address before transport."""
        with patch("cli.commands.editor.run_command") as mock_run:
            result = runner.invoke(
                cli,
                [
    ...
    def test_editor_drag_ui_by_target(self, runner, mock_unity_response):
        """Test bounded runtime UI drag command."""
        with patch(
            "cli.commands.editor.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
        ...
        assert mock_run.call_args.args[:2] == (
            ...
            {
                "action": "drag_ui",
                "target": "Canvas/Slider/Handle",
                "end_position": [0.8, 0.5],
                "steps": 8,
                "search_method": "by_path",
            },
        )

    def test_editor_scroll_ui_by_position(self, runner, mock_unity_response):
        """Test bounded runtime UI coordinate scroll command."""
        with patch(
            "cli.commands.editor.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
    ...
    def test_editor_scroll_ui_rejects_zero_delta(self, runner):
        """Reject a scroll request that cannot move anything."""
        with patch("cli.commands.editor.run_command") as mock_run:
            result = runner.invoke(
                cli,
                ["editor", "scroll-ui", "List", "--delta", "0", "0"],
            )

        assert result.exit_code != 0
        assert "cannot be 0 0" in result.output
        mock_run.assert_not_called()

    def test_editor_scroll_ui_toolkit_by_class(self, runner, mock_unity_response):
        """Test UI Toolkit scroll command."""
        with patch(
            "cli.commands.editor.run_command",
            return_value=mock_unity_response,
        ) as mock_run:
