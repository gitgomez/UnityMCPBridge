...
def mock_unity(monkeypatch):
    ...
    captured["preflight"] = preflight_mock
    return captured
...
def test_read_only_actions_do_not_refresh_dirty_assets(mock_unity, action_name):
    asyncio.run(manage_scene(SimpleNamespace(), action=action_name))
    assert mock_unity["preflight"].await_args.kwargs["refresh_if_dirty"] is False


def test_mutating_action_refreshes_dirty_assets(mock_unity):
    asyncio.run(manage_scene(SimpleNamespace(), action="save"))
    assert mock_unity["preflight"].await_args.kwargs["refresh_if_dirty"] is True
