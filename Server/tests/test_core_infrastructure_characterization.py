...
from core.telemetry import (
    TelemetryCollector, TelemetryConfig, TelemetryRecord, RecordType, MilestoneType,
...
class TestServerConfigDefaults:
    ...
    def test_config_telemetry_defaults(self):
        """Verify the fork has no default telemetry destination."""
        config = ServerConfig()

        assert config.telemetry_enabled is False
        assert config.telemetry_endpoint is None
...
class TestTelemetryConfigPrecedence:
    ...
    def test_telemetry_config_endpoint_from_server_config(self):
        """Verify explicit application config enables its own endpoint."""
        with patch("core.telemetry.import_module") as mock_import:
            mock_config = MagicMock()
    ...
    def test_telemetry_config_endpoint_validation(self):
        ...
        with patch("core.telemetry.import_module", side_effect=Exception("No module")):
            with patch("core.telemetry.TelemetryConfig._is_disabled", return_value=False):
                # Invalid endpoint must not fall back to a third-party service.
    ...
    def test_default_configuration_never_opens_http_client(self, tmp_path):
        """Default fork configuration cannot transmit telemetry."""
        with patch("core.telemetry.import_module", side_effect=Exception("No module")):
            with patch("core.telemetry.TelemetryConfig._get_data_directory", return_value=tmp_path):
                with patch("core.telemetry.httpx.Client") as http_client:
                    ...
                    try:
                        ...
                        collector._send_telemetry(record)
...
class TestTelemetryRecordTypes:
    ...
    def test_record_tool_usage_omits_error_text(self):
        """Verify record_tool_usage never records free-form error text."""
        with patch("core.telemetry.get_telemetry") as mock_get:
            mock_collector = MagicMock()
            mock_get.return_value = mock_collector
    ...
    def test_record_tool_usage_long_error_is_omitted(self):
        """Verify long error messages are omitted rather than truncated."""
        long_error = "x" * 500
    ...
    def test_record_resource_usage_omits_error_text(self):
        """Verify record_resource_usage never records free-form error text."""
        with patch("core.telemetry.get_telemetry") as mock_get:
            mock_collector = MagicMock()
            mock_get.return_value = mock_collector
