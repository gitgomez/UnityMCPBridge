...
def test_normalize_response_preserves_runtime_diagnostics():
    ...
    response = service._normalize_response(
        {
            ...
            "data": {"package": "com.example.package"},
            "code": "PACKAGE_UNAVAILABLE",
            "runtime": {"request_id": "request-1", "status": "failed"},
            "hint": "retry",
        }
    )

    assert response.code == "PACKAGE_UNAVAILABLE"
    assert response.runtime == {"request_id": "request-1", "status": "failed"}
    assert response.hint == "retry"
    assert response.data == {"package": "com.example.package"}
