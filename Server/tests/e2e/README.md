...
assertions. It is deterministic, free of LLM API dependencies, and therefore
safe to gate PRs.

`workflow_tools_smoke.py` goes one layer higher: it spawns the local FastMCP
server and uses an MCP client to exercise dependency inspection, project audit,
Input System authoring, and Addressables authoring/building. Its generated Input
System wrapper deliberately causes a domain reload and verifies that the bridge
reconnects afterward. Run it only against a disposable test project because
Addressables initialization creates persistent project settings.
Addressables `build` is asynchronous: it returns a job ID which the driver polls
through `build_status`, keeping long content builds outside the MCP request timeout.
...
uv run python tests/e2e/workflow_tools_smoke.py --instance MyProject@<hash>
```
