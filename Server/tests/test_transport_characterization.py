...
class TestPluginHubCommandRouting:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {"status": "succeeded", "message": "pong", "data": {"message": "pong"}},
            {"success": True, "message": "pong"},
        ],
    )
    def test_readiness_probe_accepts_legacy_and_runtime_pongs(self, response):
        assert PluginHub._is_successful_ping_response(response)

    @pytest.mark.parametrize(
        "response",
        [
            None,
            {"status": "error", "data": {"message": "pong"}},
            {"success": False, "message": "pong"},
            {"status": "succeeded", "message": "not-pong"},
        ],
    )
    def test_readiness_probe_rejects_failed_or_invalid_pongs(self, response):
        assert not PluginHub._is_successful_ping_response(response)
