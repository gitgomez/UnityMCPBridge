...
This server is part of the personally maintained **UnityMCPBridge** fork. See the repository [support policy](../SUPPORT.md) and [fork guide](../docs/getting-started/bridge-fork.md). The project is not affiliated with Unity Technologies.

**Required:** Install the matching [UnityMCPBridge package](../docs/getting-started/bridge-fork.md#install-the-unity-package) to connect Unity Editor with this server. The Unity package and Python server must use the same fork revision. You also need `uvx` (requires [uv](https://docs.astral.sh/uv/)) to run the server.

> **Fork distribution note:** The public `mcpforunityserver` PyPI package and `msanatan/mcp-for-unity-server` Docker image are upstream distributions. They do not contain UnityMCPBridge-only behavior. UnityMCPBridge releases source through immutable GitHub tags; use the tagged GitHub source or local-development path below.

---

## Installation

### Option 1: Upstream PyPI (not the fork)
...
### Option 2: UnityMCPBridge stable source

Use the same immutable release tag as the Unity package. This is the supported fork installation path; it does not use the upstream PyPI distribution.
...
```json
{
  "mcpServers": {
    "UnityMCP": {
      ...
      "args": [
        "--from",
        "git+https://github.com/gitgomez/UnityMCPBridge.git@v10.2.4#subdirectory=Server",
        "mcp-for-unity",
        "--transport",
        "stdio"
      ]
...
For development snapshots, replace `v10.2.4` with `optimize/bridge` in both the server source and Unity package URL. Moving-branch installations are not reproducible and can change without notice.
...
git clone https://github.com/gitgomez/UnityMCPBridge.git
cd UnityMCPBridge/Server
...
For full details, see the [Remote Server Auth Guide](../docs/guides/remote-server-auth.md) and [Architecture Reference](../docs/architecture/remote-auth.md).
...
MIT License - See [LICENSE](LICENSE). The upstream copyright notice remains in place, and fork modifications are attributed separately. Dependency-license information is included in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
