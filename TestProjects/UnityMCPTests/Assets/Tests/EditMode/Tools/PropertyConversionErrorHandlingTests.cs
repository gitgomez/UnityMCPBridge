...
namespace MCPForUnityTests.Editor.Tools
{
    ...
    public class PropertyConversionErrorHandlingTests
    {
        ...
        [Test]
        public void ManageComponents_SetProperty_ComponentReferenceArrayFromGameObjectIds()
        {
            var holder = testGameObject.AddComponent<CustomComponent>();
            var firstSource = new GameObject("FirstComponentReferenceSource");
            var secondSource = new GameObject("SecondComponentReferenceSource");

            try
            {
                var firstCollider = firstSource.AddComponent<BoxCollider>();
                var secondCollider = secondSource.AddComponent<BoxCollider>();
                ...
                var setPropertyParams = new JObject
                {
                    ...
                    ["value"] = new JArray(
                        firstSource.GetInstanceID(),
                        secondSource.GetInstanceID())
                };

                var result = ManageComponents.HandleCommand(setPropertyParams);
                var resultObject = result as JObject ?? JObject.FromObject(result);

                Assert.IsTrue(resultObject.Value<bool>("success"), resultObject.ToString());
                CollectionAssert.AreEqual(
                    new[] { firstCollider, secondCollider },
                    holder.componentReferences);
            }
            finally
            {
        ...
        [Test]
        public void ManageComponents_AcceptsHandlesForTargetAndComponentReferences()
        {
            var holder = testGameObject.AddComponent<CustomComponent>();
            var firstSource = new GameObject("FirstHandledComponentReferenceSource");
            var secondSource = new GameObject("SecondHandledComponentReferenceSource");
            ...
            try
            {
                ...
                {
                    ["action"] = "set_property",
                    ["target"] = CommandRuntimeObjectHandle.BuildSceneObjectHandle(
                        testGameObject),
                    ["componentType"] = "CustomComponent",
                    ["property"] = "componentReferences",
                    ["value"] = new JArray(
                        CommandRuntimeObjectHandle.BuildSceneObjectHandle(firstSource),
                        new JObject
                        {
                            ["handle"] = CommandRuntimeObjectHandle.BuildSceneObjectHandle(
                                secondSource)
                        })
                };
                ...
                Assert.IsTrue(resultObject.Value<bool>("success"), resultObject.ToString());
                Assert.AreEqual(
                    testGameObject.GetInstanceID(),
                    resultObject["data"]?.Value<int>("instanceID"));
                Assert.AreEqual(
                    "scene_object",
                    resultObject["data"]?["handle"]?.Value<string>("kind"));
                CollectionAssert.AreEqual(
                    new[] { firstCollider, secondCollider },
                    holder.componentReferences);
            }
