    ...
    public class ManagePrefabsStageTests
    {
        ...
        public void SavePrefabStage_PersistsPrefabContentChanges()
        {
            ...
            try
            {
                ...
                Assert.AreEqual("prefab", receipt?.Value<string>("kind"));
                Assert.AreEqual(prefabPath, receipt?.Value<string>("path"));
                Assert.Greater(receipt?.Value<long>("file_size_bytes") ?? 0L, 0L);
