...
namespace MCPForUnityTests.Editor.Tools
{
    ...
    public class GameObjectAPIStressTests
    {
        ...
        public void GameObjectResource_ReadComplexObject()
        {
            ...
            Assert.AreEqual("scene_object", data?["handle"]?["kind"]?.Value<string>());
            ...
            Assert.AreEqual(5, (data?["childHandles"] as JArray)?.Count);

            Debug.Log($"[GameObjectResource] Read complex object in {sw.ElapsedMilliseconds}ms");
        }

        [Test]
        public void GameObjectResources_AcceptSceneObjectHandles()
        {
            var go = CreateTestObject("HandledResourceTarget");
            go.AddComponent<Rigidbody>();
            JObject handle = CommandRuntimeObjectHandle.BuildSceneObjectHandle(go);

            var objectResult = ToJObject(GameObjectResource.HandleCommand(new JObject
            {
                ["target"] = handle
            }));
            var componentsResult = ToJObject(GameObjectComponentsResource.HandleCommand(
                new JObject
                {
                    ["target"] = new JObject { ["handle"] = handle },
                    ["includeProperties"] = false
                }));
            var componentResult = ToJObject(GameObjectComponentResource.HandleCommand(
                new JObject
                {
                    ["target"] = handle,
            ...
            Assert.IsTrue(objectResult.Value<bool>("success"), objectResult.ToString());
            Assert.IsTrue(componentsResult.Value<bool>("success"), componentsResult.ToString());
            Assert.IsTrue(componentResult.Value<bool>("success"), componentResult.ToString());
            Assert.AreEqual(
                go.GetInstanceID(),
                objectResult["data"]?.Value<int>("instanceID"));
            Assert.AreEqual(
                "scene_object",
                componentsResult["data"]?["gameObjectHandle"]?.Value<string>("kind"));
            Assert.AreEqual(
                "scene_object",
                componentResult["data"]?["gameObjectHandle"]?.Value<string>("kind"));
        }
