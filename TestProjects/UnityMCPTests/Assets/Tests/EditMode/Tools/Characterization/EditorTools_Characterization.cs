...
namespace MCPForUnityTests.Editor.Tools.Characterization
{
    ...
    public class EditorToolsCharacterizationTests
    {
        ...
        [Test]
        public void ErrorResponse_StableCode_IsSeparateFromReadableMessage()
        {
            var response = ErrorResponse.FromCode(
                "PACKAGE_UNAVAILABLE",
                "The requested package is not installed.",
                new { package = "com.example.package" });
            var jo = ToJO(response);

            Assert.AreEqual("PACKAGE_UNAVAILABLE", jo.Value<string>("code"));
            Assert.AreEqual("The requested package is not installed.", jo.Value<string>("error"));
            Assert.AreEqual("com.example.package", jo["data"]?.Value<string>("package"));
        }
