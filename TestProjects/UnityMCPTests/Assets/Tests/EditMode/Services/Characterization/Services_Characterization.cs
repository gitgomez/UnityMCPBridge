...
namespace MCPForUnityTests.Editor.Services.Characterization
{
    ...
    public class ServicesCharacterizationTests
    {
        ...
        [Test]
        public void EditorStateCache_PlayModeTransition_OnlySpansExitAndEnterEvents()
        {
            var handler = typeof(EditorStateCache).GetMethod(
                "OnPlayModeStateChanged",
                BindingFlags.NonPublic | BindingFlags.Static);
            Assert.IsNotNull(handler);

            var initialSnapshot = EditorStateCache.GetSnapshot();
            bool bridgeTestRunActive =
                initialSnapshot["tests"]?.Value<bool>("is_running") == true;
            string steadyPhase = bridgeTestRunActive ? "running_tests" : "idle";
            string transitionPhase =
                bridgeTestRunActive ? "running_tests" : "playmode_transition";

            void AssertTransition(bool expected, string expectedPhase)
            {
                var snapshot = EditorStateCache.GetSnapshot();
                Assert.AreEqual(
                    expected,
                    snapshot["editor"]?["play_mode"]?.Value<bool>("is_changing"));
                Assert.AreEqual(expectedPhase, snapshot["activity"]?.Value<string>("phase"));
            }

            try
            {
                handler.Invoke(null, new object[] { PlayModeStateChange.ExitingEditMode });
                AssertTransition(true, transitionPhase);

                handler.Invoke(null, new object[] { PlayModeStateChange.EnteredPlayMode });
                AssertTransition(false, steadyPhase);

                handler.Invoke(null, new object[] { PlayModeStateChange.ExitingPlayMode });
                AssertTransition(true, transitionPhase);

                handler.Invoke(null, new object[] { PlayModeStateChange.EnteredEditMode });
                AssertTransition(false, steadyPhase);
            }
            finally
            {
        ...
        [Test]
        public void EditorStateCache_TransportTelemetry_TracksConnectionAndMessages()
        {
            long before = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();

            EditorStateCache.RecordTransportConnection(true);
            EditorStateCache.RecordTransportMessage();
            var snapshot = EditorStateCache.GetSnapshot();

            Assert.IsTrue(snapshot["transport"]?.Value<bool>("unity_bridge_connected"));
            Assert.GreaterOrEqual(
                snapshot["transport"]?.Value<long>("last_message_unix_ms") ?? 0L,
                before);
        }
