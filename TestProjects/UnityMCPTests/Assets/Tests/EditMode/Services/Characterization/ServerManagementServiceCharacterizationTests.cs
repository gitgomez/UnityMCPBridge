...
namespace MCPForUnityTests.Editor.Services.Characterization
{
    ...
    public class ServerManagementServiceCharacterizationTests
    {
        ...
        private sealed class RecordingTerminalLauncher : ITerminalLauncher
        {
            ...
            private static System.Diagnostics.ProcessStartInfo HoldingStartInfo()
            {
                var psi = HarmlessNoOpStartInfo();
                psi.FileName = Application.platform == RuntimePlatform.WindowsEditor
                    ? "ping.exe"
                    : (System.IO.File.Exists("/usr/bin/sleep") ? "/usr/bin/sleep" : "/bin/sleep");
        ...
        [Test]
        public void StartLocalHttpServer_WhileManagedLaunchIsAlive_ReusesSingleLaunch()
        {
            EditorPrefs.SetBool(EditorPrefKeys.UseHttpTransport, true);
            EditorPrefs.SetString(EditorPrefKeys.HttpBaseUrl, "http://localhost:59995");
            EditorConfigurationCache.Instance.Refresh();

            var launcher = new RecordingTerminalLauncher { HoldOpen = true };
            ...
            Assert.IsTrue(service.StartLocalHttpServer(quiet: true));
            Assert.IsTrue(service.StartLocalHttpServer(quiet: true));
            Assert.AreEqual(1, launcher.HeadlessCallCount,
                "a concurrent UI/auto-start request must reuse the in-flight managed launch");
        }

        [Test]
        public void StartLocalHttpServer_WhenEndpointIsReachable_ReusesServerWithoutLaunch()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(service.StartLocalHttpServer(quiet: true));
                Assert.IsFalse(launcher.HeadlessCalled,
                    "a healthy server that survived a domain reload must not be stopped and relaunched");
            }
            finally
            {
                listener.Stop();
