...
This checkout is the maintained UnityMCPBridge fork. Normal fork work targets `optimize/bridge`; `main` is the stable public branch and must match the latest release tag. The `upstream` remote and its `beta` branch are references unless a task explicitly prepares an upstream contribution.

The Unity package (`MCPForUnity/`) and Python server (`Server/`) form one versioned bridge. Keep them on the same revision and update both sides, the CLI, tests, contracts, and relevant documentation when a capability crosses those boundaries.

Before changing a tool or runtime behavior, read:

1. `unity-mcp-skill/SKILL.md` for live capability discovery and safe Unity operation.
2. `Contracts/tool-contracts.v1.json` for the canonical machine-readable tool contract.
3. `docs/development/COMMAND_RUNTIME_V1.md` when receipts, retry, reload, handles, or command admission are involved.
4. The smallest relevant Python, C#, CLI, and test sources.

Generated tool references are outputs, not independent authorities. Validate them with:
...
Installation and operator recovery for this fork are documented in `docs/getting-started/bridge-fork.md`. Do not treat upstream package or PyPI defaults as proof that fork code is running.
...
We support Unity 6.0+ through later Unity 6.x and the CoreCLR 6.8 transition. When an API is renamed, deprecated, or removed across supported Unity 6 versions, **don't sprinkle `#if UNITY_x_y_OR_NEWER` at every call site** — add a shim in `MCPForUnity/Runtime/Helpers/Unity*Compat.cs` and route every caller through it.
...
- Don't develop directly on `main`; use `optimize/bridge` for fork work, promote a verified release commit to `main`, and branch from upstream `beta` only for an explicitly upstream-bound PR
