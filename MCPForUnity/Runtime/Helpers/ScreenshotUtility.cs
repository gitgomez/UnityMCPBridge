...
//The reason for having another Runtime Utilities in additional to Editor Utilities is to avoid Editor-only dependencies in this runtime code.
{
    ...
    public static class ScreenshotUtility
    {
        ...
        /// Falls back to camera-based capture if ScreenCapture returns null at runtime. In Editor
        /// Play Mode callers must use <see cref="CaptureCompositedAsync"/> so Unity can reach
        /// WaitForEndOfFrame without recursively pumping the PlayerLoop.
        /// </summary>
            ...
            string folderOverride = null)
        {
            ...
            if (Application.isPlaying)
            {
                throw new InvalidOperationException(
                    "Composited Play Mode capture is asynchronous. Use CaptureCompositedAsync instead.");
            }
#endif
            ...
            try
            {
                ...
                ScreenshotCaptureResult finalized = FinalizeCompositedCapture(tex, result, includeImage, maxResolution);
                tex = null; // FinalizeCompositedCapture owns and destroys the texture.
                return finalized;
            }
            finally
            {
        ...
        internal static Func<bool> RunInBackgroundGetter =
            () => Application.runInBackground;
        internal static Action<bool> RunInBackgroundSetter =
            value => Application.runInBackground = value;

        private sealed class RunInBackgroundOverrideScope : IDisposable
        {
            private readonly bool _previousRunInBackground;
            private bool _disposed;

            public RunInBackgroundOverrideScope()
            {
                _previousRunInBackground = RunInBackgroundGetter();
                ...
                try
                {
                    ...
                    {
                        RunInBackgroundSetter(true);
                    }

                    if (!RunInBackgroundGetter())
                    {
                        throw new InvalidOperationException(
                            "Could not keep the PlayerLoop active for the composited screenshot.");
                    }
                }
                catch
                {
            ...
            public void Dispose()
            {
                ...
                try
                {
                    if (RunInBackgroundGetter() != _previousRunInBackground)
                    {
                        RunInBackgroundSetter(_previousRunInBackground);
                    }
                }
                catch (Exception ex)
                {
                    Debug.LogWarning(
                        $"[MCP for Unity] Failed to restore Application.runInBackground after screenshot capture: {ex.Message}");
                }
            }
        }

        private sealed class EditorCompositedCaptureScope : IDisposable
        {
            private IDisposable _runInBackgroundOverride;

            private EditorCompositedCaptureScope()
            {
                try
                {
                    _runInBackgroundOverride =
                        new RunInBackgroundOverrideScope();
                    PrepareGameViewForCapture();
                }
                catch
                {
                    Dispose();
            ...
            public static EditorCompositedCaptureScope Begin()
            {
                if (!Application.isPlaying)
                {
                    throw new InvalidOperationException(
                        "Composited capture scope requires Play Mode.");
                }

                if (Application.isBatchMode)
                {
                    throw new InvalidOperationException(
                        "Composited Game View capture is unavailable in batch mode because WaitForEndOfFrame is not invoked.");
                }

                if (UnityEditor.EditorApplication.isPaused)
                {
                    throw new InvalidOperationException(
                        "Composited Game View capture cannot start while Play Mode is paused.");
                }
                ...
                {
                    throw new InvalidOperationException(
                        "Composited Game View capture cannot start while the Editor is compiling or updating assets.");
                }

                return new EditorCompositedCaptureScope();
            }

            public void Dispose()
            {
                _runInBackgroundOverride?.Dispose();
                _runInBackgroundOverride = null;
            }
        }

        /// <summary>
        /// Makes the Game View the active editor viewport and schedules a PlayerLoop update.
        /// This is required because WaitForEndOfFrame does not progress while the Editor is
        /// displaying the Scene View.
        /// </summary>
        internal static void PrepareGameViewForCapture()
        {
            try
            {
                if (!UnityEditor.EditorApplication.ExecuteMenuItem("Window/General/Game"))
                {
                    UnityEditor.EditorApplication.ExecuteMenuItem("Window/General/Game %2");
                }
            }
            catch
            {
                // GetWindow below remains the version-independent fallback.
            }
            ...
            {
                throw new InvalidOperationException(
                    "Unity Game View type could not be resolved for composited capture.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "Unity Game View could not be opened for composited capture.");
            }
        ...
        /// <summary>
        /// Creates the guarded Editor environment used by composited Play Mode captures.
        /// Internal for Play Mode regression tests.
        /// </summary>
        internal static IDisposable BeginEditorCompositedCaptureScope()
        {
            return EditorCompositedCaptureScope.Begin();
        }

        /// <summary>
        /// Begins the reversible background override used by Play Mode capture.
        /// Internal for deterministic Edit Mode regression tests.
        /// </summary>
        internal static IDisposable BeginRunInBackgroundOverride()
        {
            return new RunInBackgroundOverrideScope();
        }

        private static string BuildCompositedCaptureTimeoutMessage(
            double timeoutSeconds,
            int startFrame,
            bool initialRunInBackground)
        {
            int currentFrame = Time.frameCount;
            bool playerLoopAdvanced = currentFrame != startFrame;
            bool editorActive;
            try
            {
                editorActive = UnityEditorInternal.InternalEditorUtility.isApplicationActive;
            }
            catch
            {
                editorActive = UnityEditor.EditorApplication.isFocused;
            }

            string stalledReason = playerLoopAdvanced
                ? "The PlayerLoop advanced, but WaitForEndOfFrame did not complete; the Game View or graphics state may be blocked."
                : "The PlayerLoop did not advance; the Editor may be paused, blocked by modal UI, or otherwise unable to render.";

            return
                $"Composited screenshot did not complete within {Math.Max(0.1, timeoutSeconds):0.###} seconds. " +
                $"{stalledReason} " +
                $"State: frame={startFrame}->{currentFrame}, editorActive={editorActive}, " +
                $"runInBackground={Application.runInBackground} (initial={initialRunInBackground}), " +
                $"paused={UnityEditor.EditorApplication.isPaused}, " +
                $"compiling={UnityEditor.EditorApplication.isCompiling}, " +
                $"updatingAssets={UnityEditor.EditorApplication.isUpdating}.";
        }

        /// <summary>
        /// Captures the fully composited Game View after WaitForEndOfFrame. Captures are serialized
        /// so concurrent tool calls cannot overwrite shared frame state. While the capture is active,
        /// the Editor temporarily keeps the PlayerLoop running in the background and focuses the Game
        /// View; the previous background setting is restored on completion, failure, reload, or quit.
        /// </summary>
            ...
            double timeoutSeconds = 10.0)
        {
            if (!Application.isPlaying)
            {
                return CaptureComposited(
                    fileName, superSize, ensureUniqueFileName, includeImage, maxResolution, folderOverride);
            }

            await CompositedCaptureGate.WaitAsync();
            ScreenshotCapturer capturer = null;
            UnityEditor.EditorApplication.CallbackFunction pollTimeout = null;
            Texture2D tex = null;
            IDisposable captureScope = null;
            bool initialRunInBackground = Application.runInBackground;
            int startFrame = Time.frameCount;
            try
            {
                captureScope = BeginEditorCompositedCaptureScope();

                var result = PrepareCaptureResult(
                    fileName, superSize, ensureUniqueFileName, folderOverride: folderOverride, isAsync: false);
                var completion = new TaskCompletionSource<Texture2D>(
                    TaskCreationOptions.RunContinuationsAsynchronously);
                double deadline = UnityEditor.EditorApplication.timeSinceStartup
                    + Math.Max(0.1, timeoutSeconds);
                ...
                pollTimeout = () =>
                {
                    if (!Application.isPlaying)
                    {
                        completion.TrySetException(new InvalidOperationException(
                            "Play Mode ended before the composited screenshot completed."));
                    }
                    else if (UnityEditor.EditorApplication.timeSinceStartup >= deadline)
                    {
                        completion.TrySetException(new TimeoutException(
                            BuildCompositedCaptureTimeoutMessage(
                                timeoutSeconds,
                                startFrame,
                                initialRunInBackground)));
                    }
                };
                UnityEditor.EditorApplication.update += pollTimeout;

                capturer = ScreenshotCapturer.Begin(result.SuperSize, captured =>
                {
                    if (!completion.TrySetResult(captured))
                        DestroyTexture(captured);
                });

                tex = await completion.Task;
                if (tex == null)
                {
                    var cam = FindAvailableCamera();
                    if (cam != null)
                    ...
                    throw new InvalidOperationException(
                        "ScreenCapture.CaptureScreenshotAsTexture returned null and no fallback camera is available.");
                }

                ScreenshotCaptureResult finalized = FinalizeCompositedCapture(
                    tex, result, includeImage, maxResolution);
                tex = null; // FinalizeCompositedCapture owns and destroys the texture.
                return finalized;
            }
            finally
        ...
        private static ScreenshotCaptureResult FinalizeCompositedCapture(
            Texture2D tex,
    ...
    public sealed class ScreenshotCapturer : MonoBehaviour
    {
        ...
        /// <summary>Spawns a hidden GameObject, attaches a capturer, and returns its cancel handle.</summary>
        ...
        /// <summary>Cancels a pending capture and destroys its hidden host object.</summary>
        public void Cancel()
        {
            _onComplete = null;
            StopAllCoroutines();
