...
namespace MCPForUnity.Editor.Tools
{
    ...
    public static class CommandRegistry
    {
        ...
        private static bool RegisterCommandType(Type type, bool isResource)
        {
            ...
            // Keep both entry points when a tool exposes an async companion. The MCP
            // dispatcher can await frame- or reload-bound work while legacy direct callers
            // and EditMode tests can still invoke the synchronous API.
            ...
            if (asyncMethod == null && syncMethod == null)
            {
                McpLog.Warn(
                    ...
                    $"but has no public static HandleCommandAsync(JObject) or HandleCommand(JObject) method"
                );
                return false;
            }
            ...
            try
            {
                ...
                if (syncMethod != null)
                {
                    if (typeof(Task).IsAssignableFrom(syncMethod.ReturnType))
                    {
                        // Existing handlers may expose their asynchronous API under the
                        // original HandleCommand name. Preserve that supported shape.
                        asyncHandler = CreateAsyncHandlerDelegate(syncMethod, commandName);
                        syncMethod = null;
                    }
                    else
                    {
                ...
                if (asyncMethod != null)
                {
                    if (!typeof(Task).IsAssignableFrom(asyncMethod.ReturnType))
                    {
                        throw new InvalidOperationException(
                            $"HandleCommandAsync on {type.Name} must return Task or Task<T>"
                        );
                    }

                    // An explicitly named companion takes precedence over an async
                    // HandleCommand when both are present.
                    asyncHandler = CreateAsyncHandlerDelegate(asyncMethod, commandName);
                }

                _handlers[commandName] = new HandlerInfo(commandName, syncHandler, asyncHandler);
                return true;
            }
            catch (Exception ex)
            {
        ...
        /// <summary>Return whether a command is registered without invoking it.</summary>
        ...
        /// Throws only when the command has no synchronous entry point.
        /// </summary>
        ...
        public static Func<JObject, object> GetHandler(string commandName)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Command '{commandName}' has no synchronous handler and must be executed via ExecuteCommand"
                );
            }
