...
namespace MCPForUnity.Editor.Tools.GameObjects
{
    internal static class ManageGameObjectCommon
    {
        ...
        )
        {
            ...
            if (targetToken is JObject
                && CommandRuntimeObjectHandle.TryResolveSceneObject(
                    targetToken,
                    out GameObject handledObject))
            {
                results.Add(handledObject);
                return results;
            }
            ...
            switch (searchMethod)
            {
                ...
                case "by_path":
                    string normalizedSearchPath =
                        GameObjectLookup.NormalizeHierarchyPath(searchTerm);
                    if (rootSearchObject != null)
                    {
                        string rootPath = GameObjectLookup.GetGameObjectPath(rootSearchObject);
                        string relativeSearchPath = normalizedSearchPath;
                        if (normalizedSearchPath == rootPath)
                        {
                            results.Add(rootSearchObject);
                            break;
                        }
                        if (normalizedSearchPath.StartsWith(rootPath + "/", StringComparison.Ordinal))
                            relativeSearchPath = normalizedSearchPath.Substring(rootPath.Length + 1);

                        Transform foundTransform =
                            rootSearchObject.transform.Find(relativeSearchPath);
                    ...
                    else
                    {
                        ...
                        {
                            var found = GameObject.Find(normalizedSearchPath);
                            if (found != null)
                                results.Add(found);
                        }
                    }
