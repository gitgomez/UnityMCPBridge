...
namespace MCPForUnity.Editor.Tools
{
    ...
    public static class ManageEditor
    {
        ...
        // Entering or leaving play mode can reload the domain immediately. Defer the
        // transition long enough for the transport to persist and send the command
        // result first, otherwise a successful request is reported as outcome_unknown.
        private const int PlayModeTransitionDelayTicks = 2;
        private static bool? _pendingPlayModeState;
        private static int _remainingPlayModeTransitionTicks;

        internal static Func<bool> IsPlayingProvider = () => EditorApplication.isPlaying;
        internal static Action<bool> PlayModeTransitionScheduler = SchedulePlayModeTransition;
        internal static Func<string> ServerRestartScheduler = PlannedServerRestartCoordinator.Schedule;
            ...
            switch (action)
            {
                ...
                case "play":
                    try
                    {
                        ...
                        {
                            PlayModeTransitionScheduler(true);
                            return new SuccessResponse("Play mode entry requested.", new
                            {
                ...
                case "stop":
                    try
                    {
                        if (IsPlayingProvider())
                        {
                            PlayModeTransitionScheduler(false);
                            return new SuccessResponse("Play mode exit requested.", new
                            {
                ...
                {
                    string error = ServerRestartScheduler();
                    if (!string.IsNullOrEmpty(error))
                    {
                        return new ErrorResponse(error);
                    }

                    return new SuccessResponse("MCP server restart scheduled.", new
                    {
                        scheduled = true,
                        deferred = true,
                        disconnect_expected = true,
                        response_grace_ms = PlannedServerRestartCoordinator.ResponseGraceMilliseconds,
                        reconnect_automatic = true
                    });
                }
                ...
                default:
                    return new ErrorResponse(
                        $"Unknown action: '{action}'. Supported actions: play, pause, stop, set_active_tool, add_tag, remove_tag, add_layer, remove_layer, deploy_package, restore_package, restart_mcp_server, undo, redo. For prefab editing (open/save/close prefab stage), use manage_prefabs. Use MCP resources for reading editor state, project info, tags, layers, selection, windows, prefab stage, and active tool."
                    );
            }
        }

        private static void SchedulePlayModeTransition(bool shouldPlay)
        {
            _pendingPlayModeState = shouldPlay;
            _remainingPlayModeTransitionTicks = PlayModeTransitionDelayTicks;
            EditorApplication.update -= ApplyPendingPlayModeTransition;
            EditorApplication.update += ApplyPendingPlayModeTransition;
        }

        private static void ApplyPendingPlayModeTransition()
        {
            if (!_pendingPlayModeState.HasValue)
            {
                EditorApplication.update -= ApplyPendingPlayModeTransition;
                return;
            }

            _remainingPlayModeTransitionTicks--;
            if (_remainingPlayModeTransitionTicks > 0)
                return;

            bool shouldPlay = _pendingPlayModeState.Value;
            _pendingPlayModeState = null;
            EditorApplication.update -= ApplyPendingPlayModeTransition;
            if (EditorApplication.isPlaying != shouldPlay)
                EditorApplication.isPlaying = shouldPlay;
        }

        internal static void ResetPlayModeHooksForTests()
        {
            EditorApplication.update -= ApplyPendingPlayModeTransition;
            _pendingPlayModeState = null;
            _remainingPlayModeTransitionTicks = 0;
            IsPlayingProvider = () => EditorApplication.isPlaying;
            PlayModeTransitionScheduler = SchedulePlayModeTransition;
            ServerRestartScheduler = PlannedServerRestartCoordinator.Schedule;
        }
