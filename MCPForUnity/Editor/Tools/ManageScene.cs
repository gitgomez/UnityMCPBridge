...
namespace MCPForUnity.Editor.Tools
{
    ...
    public static class ManageScene
    {
        ...
        /// <summary>
        /// Dispatcher-facing companion that can wait for an end-of-frame composited screenshot.
        /// All non-screenshot actions retain the established synchronous implementation.
        /// </summary>
        public static async Task<object> HandleCommandAsync(JObject @params)
        {
            var cmd = ToSceneCommand(@params);
            if (cmd.action != "screenshot" || !RequiresAsyncCompositedCapture(cmd))
                return HandleCommand(@params);

            return await CaptureCompositedScreenshotAsync(cmd);
        }
        ...
        private static object CaptureScreenshot(SceneCommand cmd)
        {
            try
            {
                ...
                {
                    return new ErrorResponse(
                        "Composited Play Mode screenshots require the asynchronous MCP command dispatcher.");
                }
        ...
        private static bool RequiresAsyncCompositedCapture(SceneCommand cmd)
        {
            if (!Application.isPlaying || cmd.includeImage != true)
                return false;

            string captureSource = string.IsNullOrWhiteSpace(cmd.captureSource)
                ? "game_view"
                : cmd.captureSource.Trim().ToLowerInvariant();
            return captureSource == "game_view"
                && string.IsNullOrEmpty(cmd.camera)
                && string.IsNullOrEmpty(cmd.batch)
                && !cmd.viewPosition.HasValue
                && (cmd.viewTarget == null || cmd.viewTarget.Type == JTokenType.Null);
        }

        private static async Task<object> CaptureCompositedScreenshotAsync(SceneCommand cmd)
        {
            try
            {
                if (!Application.isBatchMode) EnsureGameView();
                ...
                string folderOverride = ScreenshotPreferences.Resolve(cmd.outputFolder);
                int resolvedSuperSize = cmd.superSize.HasValue && cmd.superSize.Value > 0
                    ? cmd.superSize.Value
                    : 1;
                int maxResolution = cmd.maxResolution ?? 0;
                ...
                return new SuccessResponse(
                    message,
                    BuildScreenshotResponseData(result, cameraName, includeImage: true));
            }
            catch (Exception e)
            {
                return new ErrorResponse($"Error capturing screenshot: {e.Message}");
        ...
        private static void EnsureGameView()
        {
            try
            {
                ScreenshotUtility.PrepareGameViewForCapture();
            }
            catch (Exception e)
            {
