...
namespace MCPForUnity.Editor.Tools.Cameras
{
    ...
    public static class ManageCamera
    {
        ...
        /// <summary>
        /// Dispatcher-facing companion for screenshot actions that need to wait for
        /// WaitForEndOfFrame. Other camera commands keep their synchronous behavior.
        /// </summary>
        public static async Task<object> HandleCommandAsync(JObject @params)
        {
            ...
            string action = new ToolParams(@params).Get("action")?.ToLowerInvariant();
            if (action != "screenshot" && action != "screenshot_multiview")
            ...
            return await ManageScene.HandleCommandAsync(shotParams);
        }
    }
}
