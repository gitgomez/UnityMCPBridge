    ...
    public static class GameObjectLookup
    {
        ...
        private static IEnumerable<int> SearchByPath(string path, bool includeInactive)
        {
            ...
            if (prefabStage != null)
            {
                ...
                {
                    if (MatchesPath(go, normalizedPath))
                    {
            ...
            else
            {
                ...
                var found = GameObject.Find(normalizedPath);
                if (found != null)
                {
        ...
        internal static bool MatchesPath(GameObject go, string path)
        {
            ...
            string goPath = NormalizeHierarchyPath(GetGameObjectPath(go));
            return goPath == normalizedPath
                || goPath.EndsWith("/" + normalizedPath, StringComparison.Ordinal);
        }

        /// <summary>
        /// Normalizes an external hierarchy path to Unity's root-relative forward-slash form.
        /// Handles can therefore round-trip their canonical leading-slash paths into lookup APIs.
        /// </summary>
        public static string NormalizeHierarchyPath(string path)
        {
