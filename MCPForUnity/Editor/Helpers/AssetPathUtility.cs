...
namespace MCPForUnity.Editor.Helpers
{
    ...
    public static class AssetPathUtility
    {
        ...
        /// then falls back to the matching UnityMCPBridge Git source.
        ...
        public static string GetMcpServerPackageSource()
        {
            ...
            // The fork package and server are one release unit. Never fall back to the
            // public upstream PyPI package because it does not contain fork-only tools.
            string version = GetPackageVersion();
            if (version == "unknown")
            {
                return "git+https://github.com/gitgomez/UnityMCPBridge.git@optimize/bridge#subdirectory=Server";
            }

            // Prerelease package builds follow the moving integration branch. Stable package
            // versions resolve through their immutable, matching release tag.
            if (IsSemVerPreRelease(version))
            {
                return "git+https://github.com/gitgomez/UnityMCPBridge.git@optimize/bridge#subdirectory=Server";
            }

            return $"git+https://github.com/gitgomez/UnityMCPBridge.git@v{version}#subdirectory=Server";
        }
        ...
        /// Handles explicit legacy PyPI prerelease overrides as well as Git and local sources.
        /// Centralizes source argument handling to avoid duplication between HTTP and stdio transports.
        /// Priority: explicit fromUrl override > matching fork source.
        ...
        /// <returns>The package source arguments (for example, "--from git+https://...#subdirectory=Server")</returns>
        ...
        public static string GetBetaServerFromArgs(string gitUrlOverride, string packageSource, bool quoteFromPath = false)
        {
            ...
            // Preserve support for an explicit legacy PyPI prerelease override.
        ...
        /// <summary>
        /// Determines whether uvx should bypass its reusable environment/cache.
        ...
        /// Returns isolated reinstall flags for a local source path,
        /// "--no-cache --refresh " for an explicit force refresh,
        /// "--offline " if the cache is warm, or string.Empty otherwise.
        /// Must be called on the main thread (reads EditorPrefs).
        /// </summary>
        public static string GetUvxDevFlags()
        {
            bool isLocalServerPath = IsLocalServerPath();
            bool forceRefresh = ShouldForceUvxRefresh();
            return GetUvxDevFlags(
                isLocalServerPath,
                forceRefresh,
                !forceRefresh && GetCachedOfflineProbeResult());
        }
        ...
        {
            return GetUvxDevFlags(false, forceRefresh, useOffline);
        }

        /// <summary>
        /// Pure overload used by command builders and tests. Local source trees get
        /// a fresh isolated tool environment even when their package version is unchanged.
        /// </summary>
        public static string GetUvxDevFlags(
            ...
            bool useOffline)
        {
            if (isLocalServerPath) return "--isolated --reinstall --no-cache ";
        ...
        /// <summary>
        /// Pure list overload matching GetUvxDevFlags(bool, bool, bool).
        /// </summary>
        public static IReadOnlyList<string> GetUvxDevFlagsList(
            bool isLocalServerPath,
            bool forceRefresh,
            bool useOffline)
        {
            if (isLocalServerPath)
                return new[] { "--isolated", "--reinstall", "--no-cache" };
            if (forceRefresh) return new[] { "--no-cache", "--refresh" };
            if (useOffline) return new[] { "--offline" };
