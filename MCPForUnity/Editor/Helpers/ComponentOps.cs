...
namespace MCPForUnity.Editor.Helpers
{
    ...
    public static class ComponentOps
    {
        ...
        private static bool TrySetViaReflection(object component, Type type, string propertyName, string normalizedName, BindingFlags flags, JToken value, out string error)
        {
            ...
            {
                if (RequiresSerializedObjectReferenceHandling(propInfo.PropertyType, value))
                {
                    return false;
                }
            ...
            {
                if (RequiresSerializedObjectReferenceHandling(fieldInfo.FieldType, value))
                {
                    return false;
                }
        ...
        private static bool RequiresSerializedObjectReferenceHandling(Type memberType, JToken value)
        {
            ...
            return value.Type == JTokenType.Array
                && memberType.IsArray
                && typeof(UnityEngine.Object).IsAssignableFrom(memberType.GetElementType());
        }
        ...
        private static bool SetViaSerializedProperty(Component component, string propertyName, string normalizedName, JToken value, out string error)
        {
            ...
            var verifyProp = so.FindProperty(propertyName)
                          ?? so.FindProperty(normalizedName);
            if (verifyProp != null
                && !VerifyObjectReferences(verifyProp, value, out error, 0))
            {
                return false;
            }
        ...
        private static bool VerifyObjectReferences(SerializedProperty prop, JToken value, out string error, int depth)
        {
            error = null;
            if (depth > 20)
            {
                error = "Maximum object-reference verification depth (20) exceeded.";
                return false;
            }
            ...
            {
                if (prop.arraySize != array.Count)
                {
                    error = $"Array '{prop.propertyPath}' has size {prop.arraySize} after assignment; expected {array.Count}.";
                    return false;
                }

                for (int i = 0; i < array.Count; i++)
                {
                    if (!VerifyObjectReferences(
                ...
                && value is JObject obj)
            {
                ...
                {
                    var child = FindPropertyRelativeFuzzy(prop, pair.Key);
                    if (child != null
                        && !VerifyObjectReferences(child, pair.Value, out error, depth + 1))
                    {
                        return false;
                    }
                }
            ...
            if (IsCompatibleObjectReference(prop))
                return true;

            string expectedType = ResolveObjectReferenceType(prop)?.Name ?? prop.type;
            string actualType = prop.objectReferenceValue != null
                ? prop.objectReferenceValue.GetType().Name
                : "null";
            error = $"Object reference '{prop.propertyPath}' did not persist with the expected type " +
        ...
        internal static bool SetObjectReference(SerializedProperty prop, JToken value, out string error)
        {
            ...
            if (value is JObject jObj)
            {
                ...
                // Runtime v1 scene-object handles survive scene/domain reloads when Unity
                // provides a GlobalObjectId. Wrapper objects containing a "handle" field
                // are accepted as well, matching other public GameObject target paths.
                if (CommandRuntimeObjectHandle.TryResolveSceneObject(
                        jObj,
                        out GameObject handledObject))
                {
                    return AssignObjectReference(
                        prop,
                        handledObject,
                        componentFilter,
                        out error);
                ...
                error = "Object reference must contain a scene-object handle, 'instanceID', 'guid', 'path', or 'name'.";
                return false;
            }
        ...
        private static bool AssignObjectReference(SerializedProperty prop, UnityEngine.Object resolved, string componentFilter, out string error)
        {
            ...
            var originalReference = prop.objectReferenceValue;
            ...
            if (!string.IsNullOrEmpty(componentFilter) && resolved is GameObject filterGo)
            {
                ...
                bool namedComponentFound = false;
                ...
                prop.objectReferenceValue = originalReference;
                error = namedComponentFound
                    ? $"Component '{componentFilter}' on GameObject '{filterGo.name}' is not compatible with property type '{prop.type}'."
                    : $"Component '{componentFilter}' not found on GameObject '{filterGo.name}'.";
                return false;
            }
        ...
        private static bool IsCompatibleObjectReference(SerializedProperty prop)
        {
            UnityEngine.Object assigned = prop.objectReferenceValue;
            if (assigned == null)
                return false;

            Type expectedType = ResolveObjectReferenceType(prop);
            return expectedType == null || expectedType.IsInstanceOfType(assigned);
        }
        ...
        private static Type ResolveObjectReferenceType(SerializedProperty prop)
        {
            ...
            if (string.IsNullOrEmpty(serializedType)
                || !serializedType.StartsWith(Prefix, StringComparison.Ordinal)
                || !serializedType.EndsWith(">", StringComparison.Ordinal))
            {
                return null;
            }

            string typeName = serializedType
                .Substring(Prefix.Length, serializedType.Length - Prefix.Length - 1)
                .TrimStart('$');

            return UnityTypeResolver.TryResolve(
                typeName,
