    ...
    public class McpAdvancedSection
    {
        ...
        private void InitializeUI()
        {
            ...
            if (gitUrlOverride != null)
                gitUrlOverride.tooltip = "Override server source for uvx --from. Leave empty to use the matching UnityMCPBridge Git release. Example local dev: /path/to/UnityMCPBridge/Server";
            ...
            {
                logRecordToggle.tooltip = "Log every MCP tool execution (tool, action, status, duration) to Library/UnityMCP/Logs/mcp.log.";
            ...
            {
                devModeForceRefreshToggle.tooltip = "When enabled, remote-source uvx commands add '--no-cache --refresh'. Local source paths are always launched with an isolated reinstall and no cache.";
            ...
            if (clearGitUrlButton != null)
                clearGitUrlButton.tooltip = "Clear override and use the matching UnityMCPBridge Git release";
