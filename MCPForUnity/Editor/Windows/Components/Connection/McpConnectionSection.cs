...
namespace MCPForUnity.Editor.Windows.Components.Connection
{
    ...
    public class McpConnectionSection
    {
        ...
        private void UpdateStartHttpButtonState()
        {
            ...
            bool launchPending = httpServerToggleInProgress || HttpAutoStartHandler.IsConnectPending;
            // While we are launching+connecting and the server isn't up yet, show a transient "Starting…" label.
            if (launchPending && !localServerRunning)
            {
            ...
            startHttpServerButton.SetEnabled(
                !launchPending && (shouldShowStop || canStartLocalServer));
        ...
        {
            if (httpServerToggleInProgress || HttpAutoStartHandler.IsConnectPending)
            {
                return;
            }
        ...
        private async Task TryAutoStartSessionAsync()
        {
            ...
            // Patience: keep polling reachability unless the launcher reports a non-zero exit code,
            // or a generous hard cap is reached. Exit code 0 may be a successful process hand-off.
            ...
            while (true)
            {
                ...
                bool launchFailed = server.HasManagedServerLaunchHandle
                    && server.HasManagedServerLaunchFailed();

                if (launchFailed || elapsed > hardCap.TotalSeconds)
                {
