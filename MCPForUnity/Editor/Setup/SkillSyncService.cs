...
namespace MCPForUnity.Editor.Setup
{
    ...
    {
        private const string DefaultRepoUrl = "https://github.com/gitgomez/UnityMCPBridge";
        ...
        {
            if (TryResolvePackageSkillSource(out var sourcePath))
            {
                log?.Invoke($"Using canonical skill bundled with the current package checkout: {sourcePath}");
                SyncLocalAsync(sourcePath, installDir, log, onComplete);
                return;
            }

            log?.Invoke("No local canonical skill source was found; falling back to the UnityMCPBridge repository.");
        ...
        private static void SyncLocalAsync(string sourceDir, string installDir, Action<string> log, Action<SyncResult> onComplete)
        {
            Task.Run(() =>
            {
                try
                {
                    var result = RunLocalSync(sourceDir, installDir, log);
        ...
        internal static bool TryResolvePackageSkillSource(out string sourcePath)
        {
            sourcePath = null;
            try
            {
                ...
                var candidates = new[]
                {
                    Path.Combine(packageRoot, SkillSubdir),
                    Path.Combine(packageRoot, "..", SkillSubdir),
                };
            ...
            {
                McpLog.Warn($"[SkillSync] Failed to resolve local canonical skill source: {ex.Message}");
            }
        ...
        private static SyncResult RunLocalSync(string sourceDir, string installDir, Action<string> log)
        {
            log?.Invoke("=== Local Skill Sync Start ===");

            var sourcePath = ResolveAndValidateInstallPath(sourceDir);
            if (!File.Exists(Path.Combine(sourcePath, "SKILL.md")))
            {
                throw new InvalidOperationException($"Canonical skill source is invalid: {sourcePath}");
            }

            var installPath = ResolveAndValidateInstallPath(installDir);
            var sourceComparison = GetPathComparison(sourcePath);
            if (string.Equals(
                    Path.GetFullPath(sourcePath).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar),
                    Path.GetFullPath(installPath).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar),
                    sourceComparison))
            {
                throw new InvalidOperationException("Canonical skill source and install directory must be different.");
            }

            var sourceFiles = ListFiles(sourcePath);
            if (sourceFiles.Count == 0)
            {
                throw new InvalidOperationException($"Canonical skill source contains no files: {sourcePath}");
            }

            var sourceHashes = sourceFiles.ToDictionary(
                entry => entry.Key,
                entry => ComputeGitBlobSha1(entry.Value),
                StringComparer.Ordinal);
            var sourceRevision = ComputeSourceRevision(sourceHashes);
            ...
            var installedFiles = ListFiles(installPath);
            var pathComparison = GetPathComparison(installPath);
            var pathComparer = GetPathComparer(pathComparison);
            EnsureManagedInstallRoot(installPath, installedFiles.Keys, sourceHashes.Keys, pathComparer);
            var plan = BuildPlan(sourceHashes, installedFiles, pathComparer);

            log?.Invoke($"Canonical source revision: {sourceRevision}");
            log?.Invoke($"Plan => Added:{plan.Added.Count} Updated:{plan.Updated.Count} Deleted:{plan.Deleted.Count}");
            LogPlanDetails(plan, log);

            ApplyLocalPlan(sourcePath, installPath, plan, sourceComparison, pathComparison, log);
            ValidateFileHashes(installPath, sourceHashes, pathComparison, log);
            WriteSyncMarker(installPath, sourcePath, "local", sourceRevision, SkillSubdir);

            log?.Invoke("Files mirrored from the current package checkout.");
            log?.Invoke("=== Local Skill Sync Done ===");
            return new SyncResult
            {
                Success = true,
                Added = plan.Added.Count,
        ...
        private static void ApplyLocalPlan(
            string sourceRoot,
            string targetRoot,
            SyncPlan plan,
            ...
            Action<string> log)
        {
            foreach (var relativePath in plan.Added.Concat(plan.Updated))
            {
                var sourceFile = ResolvePathUnderRoot(sourceRoot, relativePath, sourcePathComparison);
                var targetFile = ResolvePathUnderRoot(targetRoot, relativePath, targetPathComparison);
                var targetDirectory = Path.GetDirectoryName(targetFile);
                if (!string.IsNullOrEmpty(targetDirectory))
                {
                    Directory.CreateDirectory(targetDirectory);
        ...
        private static string ComputeSourceRevision(Dictionary<string, string> files)
        {
            var canonical = new StringBuilder();
            foreach (var entry in files.OrderBy(entry => entry.Key, StringComparer.Ordinal))
            {
                canonical.Append(entry.Key).Append('\0').Append(entry.Value).Append('\n');
            }
        ...
        private static void WriteSyncMarker(string installPath, string source, string branch, string revision, string subdir)
        {
            ...
            var marker = new StringBuilder()
                .AppendLine("managed-by-unity-mcp-skill-sync")
                .Append("source=").AppendLine(OneLine(source))
                .Append("branch=").AppendLine(OneLine(branch))
                .Append("revision=").AppendLine(OneLine(revision))
                .Append("subdir=").AppendLine(OneLine(subdir))
                .ToString();
            File.WriteAllText(Path.Combine(installPath, SyncOwnershipMarker), marker, Encoding.UTF8);
        }
