    ...
    internal static class HttpBridgeReloadHandler
    {
        ...
        internal static void OnBeforeAssemblyReloadCore(TransportManager transport)
        {
            if (transport.IsRunning(TransportMode.Http))
            {
                ...
                // Runtime-v1 peers use this control message to distinguish an intentional
                // domain reload from an editor shutdown or broken network connection. It is
                // best-effort so older servers keep the existing reconnect fallback.
                transport.NotifyReloading(TransportMode.Http, "assembly_reload");
