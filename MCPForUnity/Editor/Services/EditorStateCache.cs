...
namespace MCPForUnity.Editor.Services
{
    ...
    internal static class EditorStateCache
    {
        ...
        private static bool? _unityBridgeConnected;
        private static long? _lastMessageUnixMs;
        ...
        private sealed class EditorStateAssets
        {
            ...
            [JsonProperty("last_editor_save")]
            public JObject LastEditorSave { get; set; }

            [JsonProperty("editor_save_receipts")]
            public JArray EditorSaveReceipts { get; set; }
        ...
        private static void OnUpdate()
        {
            ...
            else if (_playModeTransitionPending)
            {
        ...
        {
            _playModeTransitionPending = state == PlayModeStateChange.ExitingEditMode
            ...
            var snapshot = new EditorStateSnapshot
            {
                ...
                Editor = new EditorStateEditor
                {
                    ...
                    PlayMode = new EditorStatePlayMode
                    {
                        ...
                        IsChanging = _playModeTransitionPending
                    },
                ...
                Assets = new EditorStateAssets
                {
                    ...
                    LastEditorSave = CommandRuntimeState.LastEditorSaveSnapshot(),
                    EditorSaveReceipts = CommandRuntimeState.EditorSaveReceiptsSnapshot(),
                ...
                {
                    UnityBridgeConnected = _unityBridgeConnected,
                    LastMessageUnixMs = _lastMessageUnixMs
                },
        ...
        public static JObject GetSnapshot()
        {
            lock (LockObj)
            {
                ...
                // Transport activity can be reported from background transport threads.
                // Overlay those cheap volatile values here instead of rebuilding the
                // Unity-main-thread snapshot for every message.
                if (clone["transport"] is JObject transport)
                {
                    transport["unity_bridge_connected"] = _unityBridgeConnected.HasValue
                        ? JToken.FromObject(_unityBridgeConnected.Value)
                        : JValue.CreateNull();
                    transport["last_message_unix_ms"] = _lastMessageUnixMs.HasValue
                        ? JToken.FromObject(_lastMessageUnixMs.Value)
                        : JValue.CreateNull();
                }

                // A successful resource request has reached Unity's main thread,
                // so the returned observation is current even if no tracked state
                // changed since the cached snapshot was built. Keep sequence and
                // activity timestamps stable while refreshing only response age.
        ...
        internal static void RecordTransportConnection(bool connected)
        {
            lock (LockObj)
            {
                _unityBridgeConnected = connected;
            }
        }

        internal static void RecordEditorSave()
        {
            ForceUpdate("editor_save");
        }

        internal static void RecordTransportMessage()
        {
            lock (LockObj)
            {
                _unityBridgeConnected = true;
                _lastMessageUnixMs = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
            }
        }
        ...
        // exist on the supported Unity 6 range (verified by reflection probes on 6000.0 and
