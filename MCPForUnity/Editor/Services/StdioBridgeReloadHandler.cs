...
namespace MCPForUnity.Editor.Services
{
    ...
    internal static class StdioBridgeReloadHandler
    {
        ...
        private static void CancelRetries()
        {
            ...
            try { EditorApplication.update -= ResumeTick; } catch { }
        }
        ...
        private static void OnAfterAssemblyReload()
        {
            ...
            // afterAssemblyReload fires while Unity is still finalising the new domain. Starting
            // the host synchronously here can register its EditorApplication.update pump before
            // Unity has finished rebuilding editor callbacks; the socket then listens but queued
            // commands and heartbeats are never processed. Always defer to a genuinely idle editor
            // tick, matching the HTTP reload lifecycle.
