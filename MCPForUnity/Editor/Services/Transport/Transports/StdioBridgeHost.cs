...
namespace MCPForUnity.Editor.Services.Transport.Transports
{
    ...
    public static class StdioBridgeHost
    {
        ...
        public static void Start()
        {
            lock (startStopLock)
            {
                ...
                try
                {
                    ...
                    // Re-arm the main-thread command pump on every successful start. A host can
                    // be started once during afterAssemblyReload finalisation and then restarted
                    // on a safe editor tick; Unity may discard the first callback while the static
                    // guard remains true. Remove/add is idempotent and repairs that half-running
                    // state (listening socket with no command or heartbeat processing).
        ...
        public static void Stop()
        {
            ...
            // ProcessCommands stays hooked between Stop and Start to eliminate the registration
            // gap during domain reload. Start() re-arms it idempotently after a new domain loads.
