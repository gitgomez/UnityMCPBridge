...
namespace MCPForUnity.Editor.Services.Transport
{
    ...
    public class TransportManager
    {
        ...
        /// <summary>
        /// Best-effort synchronous lifecycle notification for reload-aware transports.
        /// Returns false for legacy transports or when the signal could not be delivered.
        /// </summary>
        public bool NotifyReloading(TransportMode mode, string reason = null)
        {
            IMcpTransportClient client = GetClient(mode);
            if (client is not IReloadLifecycleTransport reloadLifecycle)
            {
                return false;
            }
            ...
            try
            {
                return reloadLifecycle.NotifyReloading(reason);
            }
            catch (Exception ex)
            {
                McpLog.Warn($"Error while notifying transport {client.TransportName} about reload: {ex.Message}");
                return false;
            }
        }
        ...
        private void UpdateState(TransportMode mode, TransportState state)
        {
            ...
            EditorStateCache.RecordTransportConnection(
                _httpState.IsConnected || _stdioState.IsConnected);
        }
    }
