...
Curated detailed examples for commonly used MCP tools. For the complete built-in
inventory, mutation class, retry semantics, and declared actions, read
[tool-contract-index.md](tool-contract-index.md). For exact parameter schemas,
use the active MCP schema or `Contracts/tool-contracts.v1.json` in a Bridge
checkout.
...
| `unityVersion` | string | e.g. `"6000.3.9f1"` |
...
| `packages.uiToolkit` | bool | Always `true` for supported Unity 6 versions (UIDocument, VisualElement, UXML/USS) |
| `packages.screenCapture` | bool | `com.unity.modules.screencapture` enabled (ScreenCapture API for screenshots) |

**Key decision points:**

- **UI system**: `packages.uiToolkit` is always true for supported Unity 6 versions; use `manage_ui` for UI Toolkit workflows (UXML/USS). If `packages.ugui` is true, use Canvas + uGUI components via `batch_execute`. UI Toolkit is preferred for new UI — it uses a frontend-like workflow (UXML for structure, USS for styling).
...
the operating-system mouse. uGUI is the default; select
`ui_system="ui_toolkit"` explicitly for a runtime `UIDocument`.

```python
# Report Play Mode state and both UI backends
interact_play_mode(action="ping")

# Inspect and wait for a uGUI control
interact_play_mode(
    action="inspect_ui",
...
# Click the raycastable center of the uGUI control
interact_play_mode(
    action="click_ui",
    target="Canvas/MainMenu/StartButton",
    search_method="by_path"
...
# Address a UI Toolkit element in a runtime UIDocument
interact_play_mode(
    action="click_ui",
...
inspection loop asynchronously, so one logical wait consumes one Runtime-v1
receipt regardless of its poll count.

uGUI pointer actions accept exactly one GameObject target or normalized Game
View position. Target lookup supports `by_id`, `by_name`, and `by_path`; the
mode is inferred when omitted. Target clicks validate layout, the runtime
`EventSystem` raycast, visibility, occlusion, interactability, and a compatible
handler.

UI Toolkit actions require a `document`. Except for `key_ui`, they use a bounded element query by name,
USS class, type, and optional index, or normalized coordinates for supported
Screen Space panels. Query support and panel-coordinate limits are documented in
[capabilities-and-limitations.md](capabilities-and-limitations.md#play-mode-ui-automation).
...
```python
interact_play_mode(
    action="hover_ui", ui_system="ui_toolkit",
    document="ManagementView", element_name="nav-market"
)
interact_play_mode(
    action="wait_ui", ui_system="ui_toolkit",
    document="ManagementView", element_name="nav-market",
    condition="hovered", expected=True
)
interact_play_mode(
    action="key_ui", ui_system="ui_toolkit",
    document="ManagementView", key_code="Escape"
)
```

`hover_ui` sends one pointer move without a click; active mouse capture or pressed
buttons are rejected. `inspect_ui.hovered` reads Unity's actual hover pseudo-state;
`hoverSupported` identifies whether that state is available. Delayed tooltips and
styles still require normal player frames before verification.

`key_ui` addresses the current focus within the specified document, or its root
when no element is focused. It does not accept element selectors or change focus.
`key_code` is one of `Escape`, `Tab`, `Return`, `Space`, `LeftArrow`, `RightArrow`,
`UpArrow`, `DownArrow`, `Backspace`, `Delete`, `Home`, `End`, `PageUp`, `PageDown`.
Optional `modifiers` contains distinct `Shift`, `Control`, `Alt`, or `Command`
values (maximum four). It sends one KeyDown/KeyUp pair with a zero character.
No navigation events are generated: Tab is not a focus-navigation command and
Return is not a submit command. If the panel detaches or focus crosses to another
document after KeyDown, the error records that KeyUp was not sent; inspect before retrying.
...
actions retain the existing runtime receipt and unsafe-without-receipt retry policy.
...
Severity follows the current Editor's recognized `LogEntry.mode` flags and only
falls back to the message body when no such flag is available. Stack-trace words
